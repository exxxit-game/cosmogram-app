...
// 20.09.2026, лента .wAuthorRibbon (index.html) — четыре захода в разговоре: (1) текст «Звезда
// автора» → «Космо-звезда» (отклонено ещё на макете), (2) 5 звёзд-рейтинга (владелец сперва
// одобрил живьём, потом сам поймал проблему — «5 звёзд это как бы об оценке»), (3) созвездие
// в один узел (макет masterskaya-lenta-interesnye-idei-20-09-2026.html, вариант «А» — «ни о
// чём не говорит как оценка, однако выделяет как символ»), (4) владелец живьём поймал реальный
// баг геометрии (SVG высотой 23px рвал границы контейнера 76×76 — «кусок залазит за текст»),
// после уменьшения высоты — «теперь почти пустая», попросил растянуть узор по всей длине
// ленты. Тот же зигзаг из 5 точек, но растянут по ширине (viewBox 104×16, было 60×26) —
// ширина ленты не была причиной бага, только высота, поэтому именно её можно занимать смело.
const WORKSHOP_RIBBON_CONSTELLATION='<svg viewBox="0 0 104 16">'+
  '<g stroke="#6b4d10" stroke-width="1" opacity=".8">'+
  '<line x1="8" y1="12" x2="28" y2="4"/><line x1="28" y1="4" x2="50" y2="11"/>'+
  '<line x1="50" y1="11" x2="72" y2="3"/><line x1="72" y1="3" x2="96" y2="9"/>'+
  '</g><g fill="#2c1f08">'+
  '<circle cx="8" cy="12" r="2.4"/><circle cx="28" cy="4" r="2"/>'+
  '<circle cx="50" cy="11" r="2.6"/><circle cx="72" cy="3" r="2"/><circle cx="96" cy="9" r="2.2"/>'+
  '</g></svg>';
...
function workshopRenderList(){
  ...
  workshopList(requestedSort).then(function(res){
    ...
    listEl.innerHTML=tracks.map(function(t){
      ...
      // 20.09.2026: текст на ленте заменён на созвездие из 5 точек (владелец решил в разговоре,
      // см. комментарий у WORKSHOP_RIBBON_CONSTELLATION выше) — L.workshopAuthorRibbon больше не читается здесь.
      '<div class="wAuthorRibbon hidden"><span>'+WORKSHOP_RIBBON_CONSTELLATION+'</span></div>'+
