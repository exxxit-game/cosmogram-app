...
const SKINS=[ // v1.44.0: палитра разведена по цветовому кругу — соседи больше не близнецы
  ...
  {id:58, name:'Квазикристалл — дифракция',    price:100, fx:'cosQuasi',    body:'#dcf6ff',fold:'#7fd0e8',glow:'rgba(70,200,230,.95)', trail:'rgba(70,200,230,', cat:'cosmos', fact:'10 лучей ровно через 36°, радиусы соседних пятен растут в φ=1.618 раз — та самая картина, которую предсказала математика Пенроуза до открытия Шехтмана (1982)'},
  {id:59, name:'Кольца Сатурна',                price:100, fx:'cosSaturn',   body:'#f5ecd6',fold:'#d4b878',glow:'rgba(230,190,120,.95)',trail:'rgba(230,190,120,', cat:'cosmos', fact:'Настоящие относительные радиусы колец NASA — щель Кассини ~4590 км, внутренние кольца честно обгоняют внешние (ω∝r⁻¹·⁵)'},
  {id:60, name:'Хвосты комет',                  price:100, fx:'cosComet',    body:'#e0eeff',fold:'#a8c4e8',glow:'rgba(150,190,255,.95)',trail:'rgba(150,190,255,', cat:'cosmos', fact:'Ионный хвост — прямая линия от Солнца (магнитное поле ветра); пылевой — изогнутая кривая (давление света + орбитальная скорость) — два разных механизма сразу'},
  {id:61, name:'Пульсар-маяк',                  price:100, fx:'cosPulsar',   body:'#e6dcff',fold:'#b0a0e8',glow:'rgba(180,160,255,.95)',trail:'rgba(180,160,255,', cat:'cosmos', fact:'Два луча с двух магнитных полюсов честно разделены 180° и обходят круг за один оборот звезды — вспышка только когда луч смотрит на наблюдателя'},
  {id:62, name:'Биполярная туманность',         price:100, fx:'cosNebula',   body:'#ffe0f5',fold:'#e08ec8',glow:'rgba(230,90,180,.95)', trail:'rgba(230,90,180,', cat:'cosmos', fact:'Быстрый звёздный ветер, сжатый медленным экваториальным поясом, раздувает две полярные доли — реальный механизм, ~13% планетарных туманностей биполярны'},
  {id:63, name:'Аврора — форма магнитосферы',   price:100, fx:'cosAurora',   body:'#dcffe8',fold:'#7ee6a8',glow:'rgba(90,220,160,.95)', trail:'rgba(90,220,160,', cat:'cosmos', fact:'Тупая подсолнечная сторона + хвост длиннее в разы (реально >200 радиусов Земли против ~10) — не декоративный овал, настоящая форма'},
  {id:64, name:'Спиральная галактика — Grand-design', price:100, fx:'cosGalaxy',     body:'#dde6ff',fold:'#9aa8e0',glow:'rgba(120,150,255,.95)',trail:'rgba(120,150,255,', cat:'cosmos', fact:'Волна плотности (Лин-Шу) держит угол закрутки ровно 15° — узор рукавов вращается как целое, не звёзды по отдельности'},
  {id:65, name:'Спиральная галактика — Flocculent',   price:100, fx:'cosGalaxyFloc', body:'#d6f0ff',fold:'#8fc4e0',glow:'rgba(100,190,230,.95)',trail:'rgba(100,190,230,', cat:'cosmos', fact:'Короткие несвязанные рукава честно вращаются с разной скоростью каждый (дифференциальная ротация) — другой реальный механизм, не Grand-design'},
  {id:66, name:'Фигуры Лихтенберга — позитивный', price:100, fx:'cosLichtPos', body:'#dceeff',fold:'#8fbfe8',glow:'rgba(100,180,240,.95)',trail:'rgba(100,180,240,', cat:'cosmos', fact:'Настоящее ветвящееся дерево (модель диэлектрического пробоя Нимайер-Пьетронеро-Вайсман, 1984) реально прорастает волной от центра к краю'},
  {id:67, name:'Фигуры Лихтенберга — негативный', price:100, fx:'cosLichtNeg', body:'#d8f0ff',fold:'#85c0e0',glow:'rgba(90,190,230,.9)', trail:'rgba(90,190,230,', cat:'cosmos', fact:'Концентрические кольца реально расходятся наружу, как круги на воде — другой знак напряжения, другая форма разряда'},
  {id:68, name:'Мыльная плёнка — законы Плато',   price:100, fx:'cosPlateau',  body:'#f0f4ff',fold:'#c8d4f0',glow:'rgba(200,210,255,.9)', trail:'rgba(200,210,255,', cat:'cosmos', fact:'Плёнки сходятся ровно тройками под 120° (честная гексагональная решётка) — доказанная геометрия минимальной поверхности, не «примерно круглое»'},
  {id:69, name:'Филлотаксис — золотой угол',     price:100, fx:'cosPhyllo',   body:'#fff3d6',fold:'#e0c080',glow:'rgba(230,190,90,.95)', trail:'rgba(230,190,90,', cat:'cosmos', fact:'Настоящая формула подсолнечника (Фогель, 1979): r=√n, θ=n·137.5077° — семена появляются одно за другим, не готовым узором'},
  {id:70, name:'Сонолюминесценция',              price:100, fx:'cosSono',     body:'#e0f4ff',fold:'#a0d0e8',glow:'rgba(140,210,240,.95)',trail:'rgba(140,210,240,', cat:'cosmos', fact:'Пузырёк расширяется в ~10 раз (4.5→45 микрон), затем резко схлопывается — вспышка света ровно в момент коллапса'},
  {id:71, name:'Аккреционный диск чёрной дыры',  price:100, fx:'cosAccretion',body:'#f4f2ff',fold:'#c9c3ea',glow:'rgba(210,200,255,.95)',trail:'rgba(210,200,255,', cat:'cosmos', fact:'Внутренние орбиты честно вращаются быстрее внешних (Кеплер, ω∝r⁻¹·⁵), джет турбулентно мерцает — не стабильный луч'},
  {id:72, name:'Волны Фарадея',                  price:100, fx:'cosFaraday',  body:'#dcf0ff',fold:'#8fc8e0',glow:'rgba(90,190,230,.95)', trail:'rgba(90,190,230,', cat:'cosmos', fact:'Параметрическая неустойчивость жидкости — честный отклик ровно на половине частоты возбуждения (субгармоника), не при любой вибрации'},
  ...
  {id:73, name:'Тонганская тапа (нгату)', price:100, fx:'culTapa', body:'#3a2210',fold:'#c98a3a',glow:'rgba(220,170,90,.95)', trail:'rgba(220,170,90,', cat:'culture', fact:'Крупный мотив «манулуа» реально появляется блок за блоком, как настоящее натирание резной доски (купеси)'},
  {id:74, name:'Андское ткачество', price:100, fx:'culAndesFront', body:'#141a3a',fold:'#c94f4f',glow:'rgba(210,90,90,.95)', trail:'rgba(210,90,90,', cat:'culture', fact:'Комплементарная перевязка нарастает снизу вверх, как на настоящем ткацком станке'},
  {id:75, name:'Палестинская татрииз', price:100, fx:'culTatreez', body:'#1a1420',fold:'#c94f4f',glow:'rgba(220,100,100,.95)', trail:'rgba(220,100,100,', cat:'culture', fact:'Счётный крест реально растёт кольцами от центра наружу, как настоящая вышивка'},
  {id:76, name:'Навахо — Two Grey Hills', price:100, fx:'culNavajoTwoGrey', body:'#cdbfa3',fold:'#5a4a30',glow:'rgba(150,120,80,.95)', trail:'rgba(150,120,80,', cat:'culture', fact:'Только натуральная небелёная шерсть — ряды реально нарастают снизу вверх на станке'},
  {id:77, name:'Навахо — Ganado', price:100, fx:'culNavajoGanado', body:'#8a1f1f',fold:'#e8ecf7',glow:'rgba(230,235,250,.95)', trail:'rgba(230,235,250,', cat:'culture', fact:'Крупные ступенчатые ромбы реально нарастают от края к центру, насыщенный красный фон'},
  {id:78, name:'Навахо — Crystal', price:100, fx:'culNavajoCrystal', body:'#9c8a5a',fold:'#4a3f28',glow:'rgba(180,160,110,.95)', trail:'rgba(180,160,110,', cat:'culture', fact:'Приглушённые растительные красители, полосы без каймы реально нарастают рядами'},
  {id:79, name:'Турецкий чинтемани', price:100, fx:'culCintemani', body:'#f0e6d2',fold:'#c94f2f',glow:'rgba(220,110,70,.95)', trail:'rgba(220,110,70,', cat:'culture', fact:'Три круга + тигровые полосы — настоящая османская имперская композиция, мотив реально оставляется резным штампом одним за другим'},
  {id:80, name:'Монгольский войлок — Өлзий', price:100, fx:'culOlzii', body:'#1c2a1c',fold:'#e8c860',glow:'rgba(230,200,100,.95)', trail:'rgba(230,200,100,', cat:'culture', fact:'Замкнутая непрерывная лента реально трассируется от начала до узла, часть «8 благих символов»'},
  {id:81, name:'Монгольский войлок — Алхан хээ', price:100, fx:'culAlkhanKhee', body:'#1c2a1c',fold:'#e8c860',glow:'rgba(230,200,100,.95)', trail:'rgba(230,200,100,', cat:'culture', fact:'Одна непрерывная открытая лента реально прокладывается, повороты только 90° — тот же принцип, что греческий меандр'},
  {id:82, name:'Эфиопский плетёный крест', price:100, fx:'culEthiopia', body:'#241a10',fold:'#d8b860',glow:'rgba(220,190,100,.95)', trail:'rgba(220,190,100,', cat:'culture', fact:'Плетение «вечность» реально прокладывается луч за лучом вокруг каждого конца креста; известно более 100 реальных региональных форм'},
  {id:83, name:'Бенгальский джамдани', price:100, fx:'culJamdani', body:'#e0dac0',fold:'#a8905a',glow:'rgba(210,190,150,.95)', trail:'rgba(210,190,150,', cat:'culture', fact:'Дополнительный уток вплетён только в местах узора — мотивы честно мерцают на просвет, как на настоящей муслиновой ткани'},
  {id:84, name:"Филиппинский т'налак", price:100, fx:'culTnalak', body:'#3a1f14',fold:'#c9a03a',glow:'rgba(210,170,80,.95)', trail:'rgba(210,170,80,', cat:'culture', fact:'Ikat — нити основы завязаны и окрашены до тканья, цвет реально проступает полосой за полосой, с размытым краем'},
  {id:85, name:'Финский рюйю', price:100, fx:'culRyijy', body:'#2a1c30',fold:'#c060a0',glow:'rgba(210,110,180,.95)', trail:'rgba(210,110,180,', cat:'culture', fact:'Длинноворсовая текстура — короткие пучки ворса честно покачиваются, симметричный свадебный мотив читается сквозь штриховку'},
  {id:86, name:'Дамасская сталь — Вуц', price:100, fx:'matWootz', body:'#1a1a1e',fold:'#b8bcc4',glow:'rgba(200,205,215,.9)', trail:'rgba(200,205,215,', cat:'culture', fact:'Волнистые полосы кристаллизации карбидов ОДНОГО сплава реально бликуют на свету'},
  {id:87, name:'Дамаскирование', price:100, fx:'matPatternWeld', body:'#1a1a1e',fold:'#c8c8d0',glow:'rgba(210,210,220,.9)', trail:'rgba(210,210,220,', cat:'culture', fact:'Параллельные слои РАЗНЫХ сталей реально проступают по мере проковки-скручивания заготовки'},
  {id:88, name:'Кольчуга — 4-в-1', price:100, fx:'matChainmail4', body:'#141418',fold:'#b8c0c8',glow:'rgba(200,208,216,.9)', trail:'rgba(200,208,216,', cat:'culture', fact:'Европейское плетение — каждое кольцо продето ровно через 4 соседних, полотно реально нарастает кольцо за кольцом'},
  {id:89, name:'Кольчуга — 6-в-1', price:100, fx:'chainmail6', body:'#141418',fold:'#c8d0d8',glow:'rgba(210,216,224,.9)', trail:'rgba(210,216,224,', cat:'culture', fact:'Каждое кольцо продето через 6 соседних — другой граф связности, не просто гуще плетение, чем 4-в-1'},
  {id:90, name:'Мраморные жилы', price:100, fx:'matMarbleVein', body:'#e8e4da',fold:'#5a5a5f',glow:'rgba(150,150,158,.9)', trail:'rgba(150,150,158,', cat:'culture', fact:'Угловатое ветвление без единой иерархии реально прорастает со временем — настоящий геологический процесс минерализации трещин'},
  {id:91, name:'Иней / дендриты', price:100, fx:'matFrost', body:'#0a1420',fold:'#a8e0f0',glow:'rgba(170,225,240,.95)', trail:'rgba(170,225,240,', cat:'culture', fact:'Гексагональная симметрия льда Ih — ветви дендрита реально прорастают наружу под 60°, не произвольным углом'},
  {id:92, name:'Панцирь черепахи', price:100, fx:'matShellHex', body:'#1a2a1a',fold:'#c9a83a',glow:'rgba(210,180,80,.95)', trail:'rgba(210,180,80,', cat:'culture', fact:'Шестиугольная решётка реально нарастает от центра кольцо за кольцом — та же геометрическая семья, что геодезический купол'},
  {id:93, name:'Перламутр', price:100, fx:'matNacre', body:'#12141c',fold:'#8ad0c8',glow:'rgba(150,215,205,.95)', trail:'rgba(150,215,205,', cat:'culture', fact:'Слои со сдвигом рядов, цвет иризации честно плывёт по углу — тот же оптический эффект, что у настоящего перламутра'},
  {id:94, name:'Суахилийская канга', price:100, fx:'culKanga', body:'#c9482f',fold:'#f0d840',glow:'rgba(240,220,80,.95)', trail:'rgba(240,220,80,', cat:'culture', fact:'Тёмная полоса — место под реальную суахилийскую пословицу (джина); кусок ткани честно наклоняется, как в руках'},
  {id:95, name:'Плетение корзин — койлинг', price:100, fx:'culBasketCoil', body:'#3a2a18',fold:'#c9a860',glow:'rgba(210,180,110,.95)', trail:'rgba(210,180,110,', cat:'culture', fact:'Спираль из сосновой хвои реально сшивается виток за витком от центра наружу'},
  {id:96, name:'Плетение корзин — плейтинг', price:100, fx:'culBasketPlait', body:'#3a2a18',fold:'#e8c878',glow:'rgba(230,205,130,.95)', trail:'rgba(230,205,130,', cat:'culture', fact:'Пальмовый лист, лента реально прокладывается через одну полосу за полосой — другая структура плетения, не просто другой узор'},
  {id:97, name:'Персидский ковёр — Тебриз', price:100, fx:'persianTabriz', body:'#7a1f1f',fold:'#e8c060',glow:'rgba(230,195,100,.95)', trail:'rgba(230,195,100,', cat:'culture', fact:'Медальон и плотные полевые цветы — ковёр реально нарастает узел за узлом снизу вверх'},
  {id:98, name:'Персидский ковёр — Исфахан', price:100, fx:'culPersianIsfahan', body:'#8a2020',fold:'#e0a0b0',glow:'rgba(225,165,180,.9)', trail:'rgba(225,165,180,', cat:'culture', fact:'Тончайший медальон с пальметтами и вазами, ковёр реально нарастает узел за узлом'},
  {id:99, name:'Персидский ковёр — Кашан', price:100, fx:'culPersianKashan', body:'#6a1a2a',fold:'#d89050',glow:'rgba(220,150,85,.9)', trail:'rgba(220,150,85,', cat:'culture', fact:'Медальон и арабески, ковёр реально нарастает узел за узлом'},
  {id:100, name:'Персидский ковёр — Кум', price:100, fx:'culPersianQom', body:'#9a3030',fold:'#e0a878',glow:'rgba(225,170,125,.9)', trail:'rgba(225,170,125,', cat:'culture', fact:'Тонкий шёлк, мелкие цветы, ковёр реально нарастает узел за узлом'},
  {id:101, name:'Персидский ковёр — Наин', price:100, fx:'culPersianNain', body:'#e8e0c8',fold:'#4a6a9a',glow:'rgba(90,120,170,.9)', trail:'rgba(90,120,170,', cat:'culture', fact:'Кремовый фон, синий медальон, ковёр реально нарастает узел за узлом'},
