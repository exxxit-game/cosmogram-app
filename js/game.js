...
/* v1.282.24 (партия 23): волна на минуте была 6, стала 5 после честных правок партий 8
   ...
   WAVE_PACE сокращает дистанцию до каждой следующей волны равномерно на 12.5% (середина
   одобренного диапазона) — форма шага (400,500,600…1000 м) сохранена, меняется только масштаб. */
const WAVE_PACE=0.875;
function waveDistTarget(m){ return WAVE_PACE*(m<=7 ? 300*m+50*m*(m+1) : 4900+1000*(m-7)); } // накопленная дистанция перехода: шаг 400,500,600…1000 м (v1.31.0), ×WAVE_PACE с партии 23
...
/* v1.282.22 «Дыхание неба» (партия 26): разбор документа про темп волн (см. Чемоданчик) нашёл,
   что три из четырёх рекомендованных рычагов уже в игре — новизна видов растёт по волнам
   (таблица весов ниже), сложность внутри волны растёт непрерывно (difficulty()), «сок»
   частично есть (тряска, впритык, полосы скорости). Не было только контраста пиков и затиший
   в духе AI Director Left 4 Dead и «кривой интереса» Шелла — единственная прежняя попытка
   (передышка после ворот, v1.282.13) была точечной заплаткой на 0.4с, не системой. WAVE_PACE
   сознательно не тронут: своя телеметрия пока ненадёжна (см. Чемоданчик — почти все свежие
   смерти с одного тестового устройства), и владелец выбрал именно этот, более дешёвый рычаг.
   lullCurve(dist) — чистая функция общей для всех дистанции (закон 12: общее небо мерят общим
   числом, не приватным вводом сида), колокол по синусу без резких щелчков на границах окна.
   lullMul() отключает передышку в Своей трассе — там частоту уже держит воля автора (customD). */
const LULL_PERIOD = 1100, LULL_SPAN = 320;
function lullCurve(dist){
  const phase = ((dist % LULL_PERIOD) + LULL_PERIOD) % LULL_PERIOD; // защита от отрицательной дистанции
  if (phase >= LULL_SPAN) return 1;
  const t = phase / LULL_SPAN;
  return 1 + Math.sin(t*Math.PI) * 0.9; // 1 → 1.9 → 1: пауза почти вдвое длиннее в пике, плавный вход/выход
}
/* v1.282.24 «Крючок открытия» (партия 28): владелец сообщил — старт стал скучным, «еду 1000
   метров, и только потом начинается сложность». Проверил по коду и симуляцией: сама по себе
   волновая математика тут ни при чём — не отличается от присланной владельцем `v1.108.0`
   (~40 спавнов на 1000м что тогда, что сейчас без «дыхания»). Виновата сама передышка партии
   26: цикл считался от S.dist=0 БЕЗ ИСКЛЮЧЕНИЙ, значит любой забег буквально въезжал в пик
   передышки на 5-6-й секунде — прямо в открытии, которое по «кривой интереса» Шелла (тот же
   документ, что породил партию 26) обязано быть самым плотным местом всего забега, не самым
   разреженным. Симуляция: 17 спавнов за первые 15с без передышки против 12 с ней. Лекарство —
   передышка не действует, пока не пройдена первая волна: открытие всегда полной плотности,
   «дыхание» начинается только с волны 2, там, где и задумывалось контрастом, а не крючком. */
function lullMul(){ return (S.mode==='custom' || S.dist<waveDistTarget(1)) ? 1 : lullCurve(S.dist); }
  ...
  if (spawnT<=0){
    spawnT = withTrack('ob', function(){
      ...
      return (lerp(.85, .26, d) * mapRand(.75,1.25) + (extraGap||0))
        * (input.useGyro ? 1/GYRO_ASSIST : 1) * (S.mode==='custom'?(S.customD||1):1) * lullMul();
    });
  }
  starT -= dt;
