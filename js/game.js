...
function initBg(){
  ...
  const total=Math.round(140*scale);
  bgStars=[];
  for(let i=0;i<total;i++) bgStars.push({x:Math.random(),y:Math.random(),z:rand(.2,1),s:rand(.5,1.8)}); // v1.38.0: 140 при эталонной ширине — «Ультра» рисует все, остальные ступени первые 90 (эталон)
}
initBg();
