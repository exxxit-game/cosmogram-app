...
const SKINS=[ // 10.09.2026: переставлено и разбито на 8 тем + classic (id0), владелец — «чтобы удобно смотреть и выбирать»
  ...
  {id:0,name:0,price:0,   body:'#efeee9',fold:'#cdcabf',glow:'rgba(230,229,225,.9)',trail:'rgba(200,198,190,', cat:'mono'},
  {id:31, name:'Дерево', price:0, fx:'matWood',   body:'#a5713a',fold:'#7c4f22',glow:'rgba(180,130,70,.9)', trail:'rgba(180,130,70,', cat:'mono'},
  {id:25, name:'Карбон', price:0, fx:'matCarbon', body:'#181a1f',fold:'#101216',glow:'rgba(90,95,105,.85)', trail:'rgba(150,155,165,', cat:'mono'},
  ...
  {id:65, name:'Спиральная галактика — Flocculent',   price:100, extra:true, fx:'cosGalaxyFloc', body:'#d6f0ff',fold:'#8fc4e0',glow:'rgba(230,140,200,.95)',trail:'rgba(230,140,200,', cat:'cosmos', fact:'Короткие несвязанные рукава честно вращаются с разной скоростью каждый (дифференциальная ротация) — другой реальный механизм, не Grand-design'}, // 10.09.2026: glow — розовые области звездообразования (H-alpha), реальный контраст с голубым корпусом, не копия
  ...
  {id:60, name:'Хвосты комет',                  price:100, extra:true, fx:'cosComet',    body:'#e0eeff',fold:'#a8c4e8',glow:'rgba(150,190,255,.95)',trail:'rgba(150,190,255,', cat:'physics', fact:'Ионный хвост — прямая линия от Солнца (магнитное поле ветра); пылевой — изогнутая кривая (давление света + орбитальная скорость) — два разных механизма сразу'},
  ...
  {id:67, name:'Фигуры Лихтенберга — негативный', price:100, extra:true, fx:'cosLichtNeg', body:'#d8f0ff',fold:'#85c0e0',glow:'rgba(90,190,230,.9)', trail:'rgba(90,190,230,', cat:'physics', fact:'Концентрические кольца реально расходятся наружу, как круги на воде — другой знак напряжения, другая форма разряда'},
  ...
  {id:70, name:'Сонолюминесценция',              price:100, extra:true, fx:'cosSono',     body:'#e0f4ff',fold:'#a0d0e8',glow:'rgba(255,255,255,.98)',trail:'rgba(255,255,255,', cat:'physics', fact:'Пузырёк расширяется в ~10 раз (4.5→45 микрон), затем резко схлопывается — вспышка света ровно в момент коллапса'}, // 10.09.2026: glow — почти белая вспышка (реальная физика: свет в момент коллапса близок к белому), не копия голубого корпуса
  ...
  {id:107, name:'Гало и ложные солнца', price:100, extra:true, fx:'cosHalo', body:'#050a14',fold:'#0f1a2c',glow:'rgba(255,210,140,.9)', trail:'rgba(255,210,140,', cat:'physics', fact:'22° — настоящий минимальный угол преломления света в гексагональном ледяном кристалле (закон Снеллиуса, n≈1.31)'},
  {id:58, name:'Квазикристалл — дифракция',    price:100, extra:true, fx:'cosQuasi',    body:'#dcf6ff',fold:'#7fd0e8',glow:'rgba(255,180,60,.95)', trail:'rgba(255,180,60,', cat:'optics', fact:'10 лучей ровно через 36°, радиусы соседних пятен растут в φ=1.618 раз — та самая картина, которую предсказала математика Пенроуза до открытия Шехтмана (1982)'}, // 10.09.2026 (владелец, живой скрин — исходный пример «сливается») — glow был той же голубой гаммой, что body, теперь тёплый янтарь, реальный спектральный контраст дифракции
  ...
  {id:104, name:'Лунный камень', price:100, extra:true, fx:'matMoonstone', body:'#e4e8f2',fold:'#c8d0e8',glow:'rgba(200,210,240,.9)', trail:'rgba(200,210,240,', cat:'optics', fact:'Адуляресценция — мягкое молочно-голубое свечение из тонких параллельных слоёв, видно почти всегда, никогда резко не гаснет'},
  ...
  {id:105, name:'Бабочка Морфо', price:100, extra:true, fx:'bioMorpho', body:'#1e4fc4',fold:'#153a94',glow:'rgba(90,150,255,.95)', trail:'rgba(90,150,255,', cat:'biology', fact:'Структурный синий цвета крыла честно не меняется с углом поворота — пигмента синего цвета у бабочки морфо физически нет вообще'},
  {id:106, name:'Жук-скарабей', price:100, extra:true, fx:'bioBeetlePol', body:'#0a1f14',fold:'#123a26',glow:'rgba(90,220,140,.9)', trail:'rgba(90,220,140,', cat:'biology', fact:'Через один фильтр круговой поляризации панцирь жука ярко-металлический, через противоположный — весь блеск исчезает, поверхность полностью чёрная'},
  {id:92, name:'Панцирь черепахи', price:100, extra:true, fx:'matShellHex', body:'#1a2a1a',fold:'#c9a83a',glow:'rgba(210,180,80,.95)', trail:'rgba(210,180,80,', cat:'biology', fact:'Шестиугольная решётка реально нарастает от центра кольцо за кольцом — та же геометрическая семья, что геодезический купол'},
  ...
  {id:87, name:'Дамаскирование', price:100, extra:true, fx:'matPatternWeld', body:'#1a1a1e',fold:'#c8c8d0',glow:'rgba(210,210,220,.9)', trail:'rgba(210,210,220,', cat:'metal', fact:'Параллельные слои РАЗНЫХ сталей реально проступают по мере проковки-скручивания заготовки'},
  {id:88, name:'Кольчуга — 4-в-1', price:100, extra:true, fx:'matChainmail4', body:'#141418',fold:'#b8c0c8',glow:'rgba(200,208,216,.9)', trail:'rgba(200,208,216,', cat:'metal', fact:'Европейское плетение — каждое кольцо продето ровно через 4 соседних, полотно реально нарастает кольцо за кольцом'},
  {id:89, name:'Кольчуга — 6-в-1', price:100, extra:true, fx:'chainmail6', body:'#141418',fold:'#c8d0d8',glow:'rgba(210,216,224,.9)', trail:'rgba(210,216,224,', cat:'metal', fact:'Каждое кольцо продето через 6 соседних — другой граф связности, не просто гуще плетение, чем 4-в-1'},
  ...
  {id:110, name:'Слоистая ламинация', price:100, extra:true, fx:'matLayeredLam', body:'#b8895a',fold:'#8f6438',glow:'rgba(200,90,50,.95)', trail:'rgba(200,90,50,', cat:'cardboard', fact:'Стилизация техники папье-крафта: форма набрана стопкой плоских срезов-контуров, каждый шов — настоящая граница между слоями'},
  {id:111, name:'Стальная стенка балки', price:100, extra:true, fx:'matSteelWall', body:'#8a94a0',fold:'#69737e',glow:'rgba(90,190,225,.95)', trail:'rgba(90,190,225,', cat:'cardboard', fact:'Реальная гофрированная стенка балки (рецензируемая статья MDPI Materials): волна заметно площе гофрокартона — отношение высоты к длине волны всего 0.14-0.18'},
  {id:112, name:'Металлический сильфон', price:100, extra:true, fx:'matBellows', body:'#b6c0ca',fold:'#8892a0',glow:'rgba(180,225,255,.98)', trail:'rgba(180,225,255,', cat:'cardboard', fact:'Реальный компенсатор US Bellows: высота 13мм, шаг 15мм — отношение 0.867, почти квадратная волна, честно U-профиль, а не синусоида'},
  {id:79, name:'Турецкий чинтемани', price:100, extra:true, fx:'culCintemani', body:'#f0e6d2',fold:'#c94f2f',glow:'rgba(220,110,70,.95)', trail:'rgba(220,110,70,', cat:'carpets', fact:'Три круга + тигровые полосы — настоящая османская имперская композиция, мотив реально оставляется резным штампом одним за другим'},
  {id:97, name:'Персидский ковёр — Тебриз', price:100, extra:true, fx:'persianTabriz', body:'#7a1f1f',fold:'#e8c060',glow:'rgba(230,195,100,.95)', trail:'rgba(230,195,100,', cat:'carpets', fact:'Медальон и плотные полевые цветы — ковёр реально нарастает узел за узлом снизу вверх'},
  {id:98, name:'Персидский ковёр — Исфахан', price:100, extra:true, fx:'culPersianIsfahan', body:'#8a2020',fold:'#e0a0b0',glow:'rgba(225,165,180,.9)', trail:'rgba(225,165,180,', cat:'carpets', fact:'Тончайший медальон с пальметтами и вазами, ковёр реально нарастает узел за узлом'},
  {id:99, name:'Персидский ковёр — Кашан', price:100, extra:true, fx:'culPersianKashan', body:'#6a1a2a',fold:'#d89050',glow:'rgba(220,150,85,.9)', trail:'rgba(220,150,85,', cat:'carpets', fact:'Медальон и арабески, ковёр реально нарастает узел за узлом'},
  {id:100, name:'Персидский ковёр — Кум', price:100, extra:true, fx:'culPersianQom', body:'#9a3030',fold:'#e0a878',glow:'rgba(225,170,125,.9)', trail:'rgba(225,170,125,', cat:'carpets', fact:'Тонкий шёлк, мелкие цветы, ковёр реально нарастает узел за узлом'},
  {id:101, name:'Персидский ковёр — Наин', price:100, extra:true, fx:'culPersianNain', body:'#e8e0c8',fold:'#4a6a9a',glow:'rgba(90,120,170,.9)', trail:'rgba(90,120,170,', cat:'carpets', fact:'Кремовый фон, синий медальон, ковёр реально нарастает узел за узлом'},
  {id:108, name:'Гирих-плитки', price:100, extra:true, fx:'culGirih', body:'#e8dfc8',fold:'#d4c8a8',glow:'rgba(190,110,70,.9)', trail:'rgba(190,110,70,', cat:'carpets', fact:'Исламская геометрия ок. 1200 г.: 10-лучевая звезда, все углы кратны 36° — те же квазикристаллические узоры, что открыл Пенроуз, за 500 лет до него'},
  {id:73, name:'Тонганская тапа (нгату)', price:100, extra:true, fx:'culTapa', body:'#3a2210',fold:'#c98a3a',glow:'rgba(220,170,90,.95)', trail:'rgba(220,170,90,', cat:'textile', fact:'Крупный мотив «манулуа» реально появляется блок за блоком, как настоящее натирание резной доски (купеси)'},
  {id:74, name:'Андское ткачество', price:100, extra:true, fx:'culAndesFront', body:'#141a3a',fold:'#c94f4f',glow:'rgba(210,90,90,.95)', trail:'rgba(210,90,90,', cat:'textile', fact:'Комплементарная перевязка нарастает снизу вверх, как на настоящем ткацком станке'},
  {id:75, name:'Палестинская татрииз', price:100, extra:true, fx:'culTatreez', body:'#1a1420',fold:'#c94f4f',glow:'rgba(220,100,100,.95)', trail:'rgba(220,100,100,', cat:'textile', fact:'Счётный крест реально растёт кольцами от центра наружу, как настоящая вышивка'},
  {id:76, name:'Навахо — Two Grey Hills', price:100, extra:true, fx:'culNavajoTwoGrey', body:'#cdbfa3',fold:'#5a4a30',glow:'rgba(150,120,80,.95)', trail:'rgba(150,120,80,', cat:'textile', fact:'Только натуральная небелёная шерсть — ряды реально нарастают снизу вверх на станке'},
  {id:77, name:'Навахо — Ganado', price:100, extra:true, fx:'culNavajoGanado', body:'#8a1f1f',fold:'#e8ecf7',glow:'rgba(230,235,250,.95)', trail:'rgba(230,235,250,', cat:'textile', fact:'Крупные ступенчатые ромбы реально нарастают от края к центру, насыщенный красный фон'},
  {id:78, name:'Навахо — Crystal', price:100, extra:true, fx:'culNavajoCrystal', body:'#9c8a5a',fold:'#4a3f28',glow:'rgba(180,90,50,.95)', trail:'rgba(180,90,50,', cat:'textile', fact:'Приглушённые растительные красители, полосы без каймы реально нарастают рядами'}, // 10.09.2026: glow — тёплый ржаво-красный, не копия оливкового корпуса
  {id:80, name:'Монгольский войлок — Өлзий', price:100, extra:true, fx:'culOlzii', body:'#1c2a1c',fold:'#e8c860',glow:'rgba(230,200,100,.95)', trail:'rgba(230,200,100,', cat:'textile', fact:'Замкнутая непрерывная лента реально трассируется от начала до узла, часть «8 благих символов»'},
  {id:81, name:'Монгольский войлок — Алхан хээ', price:100, extra:true, fx:'culAlkhanKhee', body:'#1c2a1c',fold:'#e8c860',glow:'rgba(230,200,100,.95)', trail:'rgba(230,200,100,', cat:'textile', fact:'Одна непрерывная открытая лента реально прокладывается, повороты только 90° — тот же принцип, что греческий меандр'},
  {id:82, name:'Эфиопский плетёный крест', price:100, extra:true, fx:'culEthiopia', body:'#241a10',fold:'#d8b860',glow:'rgba(220,190,100,.95)', trail:'rgba(220,190,100,', cat:'textile', fact:'Плетение «вечность» реально прокладывается луч за лучом вокруг каждого конца креста; известно более 100 реальных региональных форм'},
  {id:83, name:'Бенгальский джамдани', price:100, extra:true, fx:'culJamdani', body:'#e0dac0',fold:'#a8905a',glow:'rgba(210,190,150,.95)', trail:'rgba(210,190,150,', cat:'textile', fact:'Дополнительный уток вплетён только в местах узора — мотивы честно мерцают на просвет, как на настоящей муслиновой ткани'},
  {id:84, name:"Филиппинский т'налак", price:100, extra:true, fx:'culTnalak', body:'#3a1f14',fold:'#c9a03a',glow:'rgba(210,170,80,.95)', trail:'rgba(210,170,80,', cat:'textile', fact:'Ikat — нити основы завязаны и окрашены до тканья, цвет реально проступает полосой за полосой, с размытым краем'},
  ...
  {id:94, name:'Суахилийская канга', price:100, extra:true, fx:'culKanga', body:'#c9482f',fold:'#f0d840',glow:'rgba(240,220,80,.95)', trail:'rgba(240,220,80,', cat:'textile', fact:'Тёмная полоса — место под реальную суахилийскую пословицу (джина); кусок ткани честно наклоняется, как в руках'},
  {id:95, name:'Плетение корзин — койлинг', price:100, extra:true, fx:'culBasketCoil', body:'#3a2a18',fold:'#c9a860',glow:'rgba(210,180,110,.95)', trail:'rgba(210,180,110,', cat:'textile', fact:'Спираль из сосновой хвои реально сшивается виток за витком от центра наружу'},
  {id:96, name:'Плетение корзин — плейтинг', price:100, extra:true, fx:'culBasketPlait', body:'#3a2a18',fold:'#e8c878',glow:'rgba(230,205,130,.95)', trail:'rgba(230,205,130,', cat:'textile', fact:'Пальмовый лист, лента реально прокладывается через одну полосу за полосой — другая структура плетения, не просто другой узор'},
];
...
{id:27, name:'Улыбка', price:75, ch:'😀', cat:'faces'},
{id:28, name:'Стиль', price:75, ch:'😎', cat:'faces'},
{id:29, name:'Огонь', price:75, ch:'🔥', cat:'faces'},
{id:30, name:'Сотка', price:0, ch:'💯', cat:'faces'},
{id:31, name:'Смех', price:75, ch:'😂', cat:'faces'},
{id:32, name:'Праздник', price:75, ch:'🥳', cat:'faces'},
{id:68, name:'Взрыв мозга', price:75, ch:'🤯', cat:'faces'},
{id:69, name:'Озорство', price:75, ch:'😈', cat:'faces'},
{id:70, name:'Ниндзя', price:75, ch:'🥷', cat:'faces'},
{id:122, name:'Влюблён', price:75, ch:'😍', cat:'faces'},
{id:123, name:'В восторге', price:75, ch:'🤩', cat:'faces'},
{id:124, name:'Сон', price:75, ch:'😴', cat:'faces'},
{id:125, name:'Холод', price:75, ch:'🥶', cat:'faces'},
...
{id:577, name:'Какашка', price:75, ch:'💩', cat:'faces'},
...
{id:41, name:'Орёл', price:75, ch:'🦅', cat:'animals'},
{id:42, name:'Волк', price:75, ch:'🐺', cat:'animals'},
{id:43, name:'Лев (зверь)', price:75, ch:'🦁', cat:'animals'},
{id:44, name:'Лиса', price:75, ch:'🦊', cat:'animals'},
{id:78, name:'Тигр', price:75, ch:'🐯', cat:'animals'},
{id:79, name:'Акула', price:75, ch:'🦈', cat:'animals'},
{id:80, name:'Сова', price:75, ch:'🦉', cat:'animals'},
{id:81, name:'Медведь', price:75, ch:'🐻', cat:'animals'},
{id:82, name:'Панда', price:75, ch:'🐼', cat:'animals'},
{id:83, name:'Слон', price:75, ch:'🐘', cat:'animals'},
{id:84, name:'Носорог', price:75, ch:'🦏', cat:'animals'},
{id:85, name:'Жираф', price:75, ch:'🦒', cat:'animals'},
{id:86, name:'Зебра', price:75, ch:'🦓', cat:'animals'},
{id:87, name:'Олень', price:75, ch:'🦌', cat:'animals'},
{id:88, name:'Леопард', price:75, ch:'🐆', cat:'animals'},
{id:89, name:'Летучая мышь', price:75, ch:'🦇', cat:'animals'},
{id:90, name:'Крокодил', price:75, ch:'🐊', cat:'animals'},
{id:91, name:'Скорпион', price:75, ch:'🦂', cat:'animals'},
{id:92, name:'Паук', price:75, ch:'🕷️', cat:'animals'},
{id:93, name:'Ящерица', price:75, ch:'🦎', cat:'animals'},
{id:94, name:'Осьминог', price:75, ch:'🐙', cat:'animals'},
{id:95, name:'Кальмар', price:75, ch:'🦑', cat:'animals'},
{id:96, name:'Кит', price:75, ch:'🐋', cat:'animals'},
{id:97, name:'Дельфин', price:75, ch:'🐬', cat:'animals'},
{id:98, name:'Тираннозавр', price:75, ch:'🦖', cat:'animals'},
{id:99, name:'Динозавр', price:75, ch:'🦕', cat:'animals'},
{id:100, name:'Павлин', price:75, ch:'🦚', cat:'animals'},
{id:101, name:'Фламинго', price:75, ch:'🦩', cat:'animals'},
{id:102, name:'Пчела', price:75, ch:'🐝', cat:'animals'},
{id:103, name:'Бабочка', price:75, ch:'🦋', cat:'animals'},
{id:186, name:'Ленивец', price:75, ch:'🦥', cat:'animals'},
{id:187, name:'Ёж', price:75, ch:'🦔', cat:'animals'},
{id:188, name:'Черепаха', price:75, ch:'🐢', cat:'animals'},
{id:189, name:'Попугай', price:75, ch:'🦜', cat:'animals'},
...
{id:137, name:'Пицца', price:75, ch:'🍕', cat:'food'},
{id:138, name:'Пончик', price:75, ch:'🍩', cat:'food'},
{id:139, name:'Мороженое', price:75, ch:'🍦', cat:'food'},
{id:140, name:'Арбуз', price:75, ch:'🍉', cat:'food'},
{id:141, name:'Бургер', price:75, ch:'🍔', cat:'food'},
{id:142, name:'Тако', price:75, ch:'🌮', cat:'food'},
{id:143, name:'Вишня', price:75, ch:'🍒', cat:'food'},
{id:144, name:'Шоколад', price:75, ch:'🍫', cat:'food'},
...
{id:33, name:'Сакура', price:75, ch:'🌸', cat:'nature'},
{id:34, name:'Кактус', price:75, ch:'🌵', cat:'nature'},
{id:35, name:'Пальма', price:75, ch:'🌴', cat:'nature'},
{id:36, name:'Клевер', price:75, ch:'🍀', cat:'nature'},
{id:71, name:'Роза', price:75, ch:'🌹', cat:'nature'},
{id:72, name:'Гриб', price:75, ch:'🍄', cat:'nature'},
{id:126, name:'Подсолнух', price:75, ch:'🌻', cat:'nature'},
{id:127, name:'Клён', price:75, ch:'🍁', cat:'nature'},
{id:182, name:'Гибискус', price:75, ch:'🌺', cat:'nature'},
{id:183, name:'Колос', price:75, ch:'🌾', cat:'nature'},
{id:175, name:'Вулкан', price:75, ch:'🌋', cat:'nature'},
{id:176, name:'Гора', price:75, ch:'🏔️', cat:'nature'},
...
{id:360, name:'Глобус: Европа-Африка', price:75, ch:'🌍', cat:'nature'},
{id:361, name:'Глобус: Америка', price:75, ch:'🌎', cat:'nature'},
{id:362, name:'Глобус: Азия-Австралия', price:75, ch:'🌏', cat:'nature'},
{id:363, name:'Глобус с меридианами', price:75, ch:'🌐', cat:'nature'},
{id:366, name:'Гора (вектор эмодзи)', price:75, ch:'⛰️', cat:'nature'},
{id:368, name:'Фудзияма', price:75, ch:'🗻', cat:'nature'},
{id:369, name:'Кемпинг', price:75, ch:'🏕️', cat:'nature'},
...
{id:1, name:'Ракета', price:75, ch:'🚀', cat:'space'},
{id:2, name:'Тарелка', price:75, ch:'🛸', cat:'space'},
{id:3, name:'Звезда', price:0, ch:'🌟', cat:'space'},
{id:4, name:'Комета', price:75, ch:'☄️', cat:'space'},
{id:6, name:'Полумесяц', price:75, ch:'🌙', cat:'space'},
{id:7, name:'Пришелец', price:75, ch:'👽', cat:'space'},
{id:8, name:'Искра', price:75, ch:'✨', cat:'space'},
{id:62, name:'Спутник', price:75, ch:'🛰️', cat:'space'},
{id:64, name:'Телескоп', price:75, ch:'🔭', cat:'space'},
{id:45, name:'Новолуние', price:75, ch:'🌑', cat:'space'},
{id:46, name:'Растущий серп', price:75, ch:'🌒', cat:'space'},
{id:47, name:'Первая четверть', price:75, ch:'🌓', cat:'space'},
{id:48, name:'Растущая Луна', price:75, ch:'🌔', cat:'space'},
{id:49, name:'Полнолуние', price:75, ch:'🌕', cat:'space'},
{id:50, name:'Убывающая Луна', price:75, ch:'🌖', cat:'space'},
{id:51, name:'Последняя четверть', price:75, ch:'🌗', cat:'space'},
{id:52, name:'Убывающий серп', price:75, ch:'🌘', cat:'space'},
{id:699, name:'Сатурн', price:75, ch:'🪐', cat:'space'},
{id:59, name:'Алмаз', price:75, ch:'💎', cat:'symbols'},
{id:60, name:'Корона', price:75, ch:'👑', cat:'symbols'},
{id:61, name:'Мешок звёзд', price:75, ch:'💰', cat:'symbols'},
{id:113, name:'Кубок', price:75, ch:'🏆', cat:'symbols'},
{id:115, name:'Кольцо', price:75, ch:'💍', cat:'symbols'},
{id:116, name:'Трезубец', price:75, ch:'🔱', cat:'symbols'},
{id:118, name:'Лилия', price:75, ch:'⚜️', cat:'symbols'},
{id:170, name:'Внимание', price:75, ch:'⚠️', cat:'symbols'},
{id:171, name:'Радиация', price:75, ch:'☢️', cat:'symbols'},
{id:172, name:'Биоопасность', price:75, ch:'☣️', cat:'symbols'},
{id:173, name:'Хрустальный шар', price:75, ch:'🔮', cat:'symbols'},
{id:174, name:'Компас', price:75, ch:'🧭', cat:'symbols'},
{id:700, name:'Монета', price:75, ch:'🪙', cat:'symbols'},
{id:1017, name:'Военная медаль', price:75, ch:'🎖️', cat:'symbols'},
{id:1018, name:'Спортивная медаль', price:75, ch:'🏅', cat:'symbols'},
{id:1019, name:'Медаль за 1 место', price:75, ch:'🥇', cat:'symbols'},
{id:1020, name:'Медаль за 2 место', price:75, ch:'🥈', cat:'symbols'},
{id:1021, name:'Медаль за 3 место', price:75, ch:'🥉', cat:'symbols'},
...
{id:21, name:'Молния', price:75, ch:'⚡', cat:'weather'},
{id:22, name:'Радуга', price:75, ch:'🌈', cat:'weather'},
{id:23, name:'Снежинка', price:75, ch:'❄️', cat:'weather'},
{id:24, name:'Волна', price:75, ch:'🌊', cat:'weather'},
{id:25, name:'Смерч', price:75, ch:'🌪️', cat:'weather'},
{id:26, name:'Солнце', price:75, ch:'☀️', cat:'weather'},
{id:66, name:'Циклон', price:75, ch:'🌀', cat:'weather'},
{id:181, name:'Термометр', price:75, ch:'🌡️', cat:'weather'},
...
{id:145, name:'Футбол', price:75, ch:'⚽', cat:'sport'},
{id:146, name:'Баскетбол', price:75, ch:'🏀', cat:'sport'},
{id:147, name:'Боулинг', price:75, ch:'🎳', cat:'sport'},
{id:148, name:'Бокс', price:75, ch:'🥊', cat:'sport'},
{id:149, name:'Регби', price:75, ch:'🏈', cat:'sport'},
{id:150, name:'Теннис', price:75, ch:'🎾', cat:'sport'},
{id:151, name:'Волейбол', price:75, ch:'🏐', cat:'sport'},
{id:152, name:'Скейт', price:75, ch:'🛹', cat:'sport'},
...
{id:53, name:'Нота', price:75, ch:'🎵', cat:'music'},
{id:54, name:'Гитара', price:75, ch:'🎸', cat:'music'},
{id:55, name:'Наушники', price:75, ch:'🎧', cat:'music'},
{id:104, name:'Пианино', price:75, ch:'🎹', cat:'music'},
{id:105, name:'Барабан', price:75, ch:'🥁', cat:'music'},
{id:106, name:'Труба', price:75, ch:'🎺', cat:'music'},
{id:107, name:'Скрипка', price:75, ch:'🎻', cat:'music'},
...
{id:37, name:'Пешка', price:75, ch:'♟️', cat:'games'},
{id:39, name:'Пики', price:75, ch:'♠️', cat:'games'},
{id:40, name:'Червы', price:75, ch:'♥️', cat:'games'},
{id:74, name:'Бубны', price:75, ch:'♦️', cat:'games'},
{id:75, name:'Трефы', price:75, ch:'♣️', cat:'games'},
{id:76, name:'Кости', price:75, ch:'🎲', cat:'games'},
{id:77, name:'Мишень', price:75, ch:'🎯', cat:'games'},
{id:184, name:'Джокер', price:75, ch:'🃏', cat:'games'},
{id:185, name:'Слот-машина', price:75, ch:'🎰', cat:'games'},
...
{id:196, name:'Ёлка', price:75, ch:'🎄', cat:'holidays'},
{id:197, name:'Тыква', price:75, ch:'🎃', cat:'holidays'},
{id:199, name:'Шарик', price:75, ch:'🎈', cat:'holidays'},
{id:200, name:'Подарок', price:75, ch:'🎁', cat:'holidays'},
{id:201, name:'Свеча', price:75, ch:'🕯️', cat:'holidays'},
...
{id:204, name:'Колесо обозрения', price:75, ch:'🎡', cat:'landmarks'},
{id:205, name:'Горки', price:75, ch:'🎢', cat:'landmarks'},
...
{id:56, name:'Дракон', price:75, ch:'🐉', cat:'myth'},
{id:57, name:'Единорог', price:75, ch:'🦄', cat:'myth'},
{id:58, name:'Дракон (лицо)', price:75, ch:'🐲', cat:'myth'},
{id:108, name:'Призрак', price:75, ch:'👻', cat:'myth'},
{id:109, name:'Джинн', price:75, ch:'🧞', cat:'myth'},
{id:110, name:'Русалка', price:75, ch:'🧜', cat:'myth'},
{id:111, name:'Демон', price:75, ch:'👹', cat:'myth'},
