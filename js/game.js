...
const SKINS=[ // v1.44.0: палитра разведена по цветовому кругу — соседи больше не близнецы
  ...
  // сохранены в .knowledge/archive-premium-skins-id9-14-08-09-2026.md).
  // 09.09.2026 (владелец, найдено по его же вопросу): id9-14 ПЕРЕНЕСЕНЫ на id52-57 — те же
  // id раньше принадлежали старым Спутники/Грани/Инкрустация/Филигрань/Ядро/Прицел, и владение
  // хранится ТОЛЬКО по числу (ownedSkins:[...id]), без привязки к содержимому — старые
  // покупатели (даже тестовые) автоматически "владели" бы и новым содержимым на том же id.
  // Правило на будущее: новой теме — всегда новый id, проданный id никогда не переиспользуется
  // (см. .knowledge/AI-DECISION-REGISTRY.md).
  {id:52, name:'Спиральная галактика',       price:100, premium:true, fx:'cosGalaxy',    body:'#dde6ff',fold:'#9aa8e0',glow:'rgba(120,150,255,.95)',trail:'rgba(120,150,255,', cat:'stars'}, // Волна плотности вращается как целое (Лин-Шу), не звёзды по отдельности
  {id:53, name:'Аккреционный диск',          price:100, premium:true, fx:'cosAccretion', body:'#f4f2ff',fold:'#c9c3ea',glow:'rgba(210,200,255,.95)',trail:'rgba(210,200,255,', cat:'stars'}, // Кеплеровская дифф. ротация + турбулентно мерцающий джет
  {id:54, name:'Квазикристалл',              price:100, premium:true, fx:'cosQuasi',     body:'#ffe0ec',fold:'#e592b0',glow:'rgba(255,90,140,.95)', trail:'rgba(255,90,140,', cat:'stars'},  // Апериодичная диаграмма дифракции, 10-лучевая симметрия
  {id:55, name:'Дамасская сталь',            price:100, premium:true, fx:'matWootz',     body:'#fff0d6',fold:'#e0b46a',glow:'rgba(230,170,70,.95)', trail:'rgba(230,170,70,', cat:'stars'},  // Вуц — блеск бежит по волнистым полосам «воды»
  {id:56, name:'Кольчуга',                   price:100, premium:true, fx:'chainmail6',   body:'#d8ffe8',fold:'#8ed9ac',glow:'rgba(70,220,130,.95)', trail:'rgba(70,220,130,', cat:'stars'},  // 6-в-1, полотно реально собирается кольцо за кольцом
  {id:57, name:'Персидский ковёр',           price:100, premium:true, fx:'persianTabriz',body:'#d2f6ff',fold:'#7fc9e0',glow:'rgba(60,190,230,.95)', trail:'rgba(60,190,230,', cat:'stars'},  // Тебриз — ткётся узел за узлом снизу вверх
  // trailFx убран 08.09.2026: мёртвое поле с 05.09.2026 — след теперь независимый выбор игрока
  // (render.js:2313), skin.trailFx нигде не читается, старые темы копировали его по инерции
  ...
  {id:45, name:45, price:2500, fx:'patPenrose',   body:'#efeee9',fold:'#cdcabf',glow:'rgba(167,139,250,.95)', trail:'rgba(167,139,250,', cat:'patterns'}, // Пенроуз — настоящая мозаика де Брёйна, проверена скриптом (одна длина стороны, 2 угла у всех 40 ромбов)
  {id:46, name:46, price:2500, fx:'patLattice2',  body:'#efeee9',fold:'#cdcabf',glow:'rgba(167,139,250,.95)', trail:'rgba(167,139,250,', cat:'patterns'}, // Цветочная решётка
  {id:47, name:47, price:2500, fx:'patCircles',   body:'#efeee9',fold:'#cdcabf',glow:'rgba(167,139,250,.95)', trail:'rgba(167,139,250,', cat:'patterns'}, // Плед из кругов
  {id:48, name:48, price:2500, fx:'illCrystal',   body:'#efeee9',fold:'#cdcabf',glow:'rgba(167,139,250,.95)', trail:'rgba(167,139,250,', cat:'patterns'}, // Кристалл — гранёный корпус вместо гладкого металла/бумаги
  ...
  {id:49, name:'Кольца Сатурна', price:180, premium:true, fx:'cosSaturn', body:'#efeee9',fold:'#cdcabf',glow:'rgba(230,190,120,.95)', trail:'rgba(230,190,120,', cat:'cosmos'}, // Настоящие относительные радиусы колец (NASA), честная кеплеровская дифференциальная ротация — внутренние обгоняют внешние. Цена уточнена 08.09: 180 Stars, без манипулятивных «99-окончаний»
  {id:50, name:'Форма капли, аврора мерцает', price:2200, fx:'cosAurora', body:'#efeee9',fold:'#cdcabf',glow:'rgba(90,220,160,.95)', trail:'rgba(90,220,160,', cat:'cosmos'}, // Настоящая форма магнитосферы — тупая подсолнечная сторона, длинный хвост, честное мерцание высыпания частиц
  {id:51, name:'Вращающийся маяк (пульсар)', price:2000, fx:'cosPulsar', body:'#efeee9',fold:'#cdcabf',glow:'rgba(180,160,255,.95)', trail:'rgba(180,160,255,', cat:'cosmos'} // Два луча с двух магнитных полюсов, честно разделены 180°, полный оборот за цикл — настоящий маяк-эффект
];
