...
let audioCreateFailReported=false; // 23.09.2026: младший брат audioResumeFailReported ниже
...
function audio(){ // создавать/возобновлять строго по жесту
  ...
  /* 23.09.2026 (аудит видимости ошибок, владелец: «нужно это всё исправлять»): тот же диагноз,
     что уже 24.08.2026 лечили ниже у resume() («музыка не работает» — 0 сигналов за 10 дней,
     потому что отказ глотался молча) — только на шаг раньше. Если САМ конструктор бросает
     (старый браузер без AudioContext, нехватка ресурсов), AC остаётся null навсегда, и КАЖДАЯ
     последующая попытка resume() ниже просто не выполняется (if(AC && ...) — AC ложный) —
     тот же результат «музыки нет», но даже audio_resume_fail не сработает ни разу, сигнала
     не было вообще ни одного. Один раз за сессию, тот же формат сообщения, что у соседа. */
  if(!AC){ try{ AC=new (window.AudioContext||window.webkitAudioContext)(); }catch(e){
    if(!audioCreateFailReported){ audioCreateFailReported=true;
      try{ if(typeof BEACON!=='undefined' && BEACON.signal) BEACON.signal('audio_create_fail', String((e&&e.name)||e||'?').slice(0,60)); }catch(_){}
    }
  } }
...
const GAME_VERSION = '1.478.561'; // «Об игре» в настройках — при репортах багов спрашивать её; «Рассвет космоса»
