...
const Store = {
  ...
  CLOUD_KEYS:['best','wallet','ownedSkins','skin','ownedDecals','decal','ownedIcons','icon','ownedFlashes','flash',
              'favDecal','favIcon','favFlash','savedRun','stats','refBy',
  ...
  _mergeDisk(keep){
    ...
    for(const k in disk){
      ...
      else if((k==='ownedSkins'||k==='ownedDecals'||k==='ownedIcons'||k==='ownedFlashes'||k==='ach') && Array.isArray(nv) && Array.isArray(cur)) this.mem[k]=[...new Set(cur.concat(nv))];
      else this.mem[k]=nv;
    }
  },
  _mergeCloud(res){
    for(const k in res){
      ...
      else if((k==='ownedSkins'||k==='ownedDecals'||k==='ownedIcons'||k==='ownedFlashes') && Array.isArray(nv) && Array.isArray(cur)) this.mem[k]=[...new Set(cur.concat(nv))]; // купленное не пропадает
...
const GAME_VERSION='1.477.73'; // «Об игре» в настройках — при репортах багов спрашивать её; «Рассвет космоса»
