...
// 03.09.2026: Спидран остался ежедневным (владелец — «Небо месяца» касается только
// Трассы дня, Спидран в этом разговоре не упоминался) — ему нужен настоящий
// ежедневный UTC-ключ, тот же расчёт, каким раньше был сам trackDayKey() до правки
// выше. Отдельная функция, а не общая: молчаливая связка через одно и то же имя —
// ровно то, из-за чего Спидран чуть не стал месячным незаметно вместе с Трассой дня.
function utcDayKey(){ const d=new Date(); return d.getUTCFullYear()+'-'+String(d.getUTCMonth()+1).padStart(2,'0')+'-'+String(d.getUTCDate()).padStart(2,'0'); }
...
const GAME_VERSION='1.478.55'; // «Об игре» в настройках — при репортах багов спрашивать её; «Рассвет космоса»
