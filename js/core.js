...
// 06.09.2026 «Слалом»: тот же приём Set Seed, что у Спидрана выше — одна трасса навсегда,
// свой собственный якорь-дата, чтобы не столкнуться с speedrun_runs по ключу day.
const SLALOM_ETERNAL_DAY='2000-01-02';
// 06.09.2026 «Биатлон»: тот же приём, свой якорь — не путать с speedrun_runs/slalom_runs.
const BIATHLON_ETERNAL_DAY='2000-01-03';
/* 11.09.2026 «Speedrun RSG»: второй, ДОПОЛНИТЕЛЬНЫЙ вариант Спидрана — не замена SSG
   (SPEEDRUN_ETERNAL_DAY выше остаётся как есть, свой честный рекорд). RSG берёт свежий
   случайный сид каждый забег (как Caravan/Слалом/Биатлон), но живёт в той же таблице
   speedrun_runs — этот якорь просто второй допустимый «day», не настоящая дата и не
   привязан к сиду напрямую (сервер сид никогда не хранил и не проверял, см. cosmogram-daily). */
const SPEEDRUN_RSG_DAY='2000-01-04';
...
const GAME_VERSION = '1.478.406'; // «Об игре» в настройках — при репортах багов спрашивать её; «Рассвет космоса»
/* 11.09.2026 «Разбивка взлёта»: живой отчёт с Samsung A3 Core показал зонд дребезга
   ...
   startGame() (ui.js), выполняющихся следом (AudioContext, keepAwake, мост Telegram gyroKick,
   калибровка датчика). Одно число не говорило, где именно — отдельные замеры вместо гадания.
   Владелец (перед волной тестов на разных устройствах, «от телефона до телевизора»): список
   довести до максимума сразу, не догонять новыми полями письмо за письмом. Читаются
   deviceProfileProbe()'ом уже ПОСЛЕ того, как реальный взлёт их заполнил (её собственный
   promise ждёт 2.5 реальных секунды, синхронный код startGame() успевает целиком раньше). */
const TAKEOFF_T={audio:0, wake:0, gyro:0, cal:0};
...
/* 05.09.2026: раньше — отдельный тумблер в Настройках (Store.get('morseOn')), спрятанный
   и выключенный по умолчанию. Владелец: тумблер убрать, Морзянка переезжает во вкладку
   «След» целиком — там её проще найти и понять, что это. Сама функция и все её вызовы
   (game.js, render.js) не тронуты — проверяют теперь выбранный след, а не старый Store-ключ. */
function morseOn(){ return typeof S!=='undefined' && typeof TRAILS_BY_ID!=='undefined' && (TRAILS_BY_ID.get(S.trail)||{}).style==='morse'; }
...
function streakDayCheck(){ // v1.108.1 «Серия дней»: sfx.streak() существовал с самого начала («тёплый огонёк
  ...
  // 18.09.2026 (сквозная проверка всей игры, владелец, явное «да» на правку ядра): streakCount
  // читался без saneNumber, в отличие от daysTotal тут же рядом (строка ниже) — испорченное/
  // чужое значение из хранилища (та же цена мержа между устройствами, что уже разбирали у
  // wallet) могло бы склеиться строкой (count+1 → конкатенация) вместо числа. saneNumber —
  // тот же самый защитный приём, что уже стоит у daysTotal в соседней строке.
  if(last===k) return saneNumber(Store.get('streakCount',0),0); // уже сегодня — просто вернуть текущий счёт, ничего не менять
  const y=new Date(); y.setDate(y.getDate()-1);
  const cont = last===dateKey(y); // вчера играл — серия продолжается; иначе — начинается заново
  const count = cont ? (saneNumber(Store.get('streakCount',0),0)+1) : 1;
...
function tgInsetsSync(){ // v1.59.0 «Подушка»: безопасная зона в CSS-переменные --js-sat/--js-sab
  ...
  /* 18.09.2026 (владелец, живой Samsung SM-A032F + Oppo CPH2631: «кнопка "Отправить Stars"
     выходит за экран») — тот же самый «мёртвый ноль», что уже документирован у верха/права
     ЗДЕСЬ ЖЕ, только у низа (жест-полоса Android) и БЕЗ условия !cgFs: живой замер показал
     contentSafeAreaInset.bottom=0 даже в полноэкранном режиме (isFullscreen:true), где верх
     при этом честно пришёл 46 — значит у низа своя, отдельная дыра, не совпадающая с уже
     изученным условием верха/права. 48px — та же оценка по минимальному касанию Android
     (48dp), что уже даёт 44px у права; требует подтверждения на реальном устройстве, как и
     та правка — если не хватит или будет с запасом, перепроверить. */
  if (t && ('ontouchstart' in window) && bot<24) bot=48;
  ...
  // 15.09.2026 (владелец, живой отчёт iPhone 16 Pro: HUD «смещён вниз», tgv:7.10 exp:1 full:0,
  // sat все сырые составляющие нулевые): «сырой верх = 0» раньше трактовался как «наверное,
  // чёлка, мост просто не умеет её измерить» — и получал ту же большую защитную добавку
  // (index.html: .island #hud{padding-top:max(206px, calc(150px+--sat))}), что и настоящий
  // fullscreen-вырез. Но rawTop=0 у старого моста (< 8.0, requestFullscreen даже не вызывается,
  // см. tgImmersion выше) БЕЗ fullscreen — это не чёлка под канвасом, это родная шапка Telegram
  // САМА стоит сверху и сама прикрывает вырез; добавочный клиренс тут не нужен вовсе, отсюда и
  // «HUD уехал вниз» (246px вместо ~96px). Измеренный вырез (rawTop≥40) остаётся доверенным
  // всегда — это настоящая цифра, не догадка. «Нет данных» (rawTop≤0) доверяем только когда
  // честно в fullscreen (канвас правда рисуется под системным UI, клиренс нужен) — страж
  // guardIslandNeedsRealNotchOrFullscreen (tests/guard.mjs) проверяет все четыре сочетания.
  try{
    ...
    const realNotch = rawTop>=40;
    const unknownButFullscreen = rawTop<=0 && !!(t&&t.isFullscreen);
    document.documentElement.classList.toggle('island', ios && (realNotch || unknownButFullscreen));
  }catch(e){}
  ...
  // 16.09.2026: тот же повторный пересчёт нужен заголовкам меню-экранов (js/ui.js retitleScreen) —
  // --sat-menu зависит от --js-sat-real, только что обновлённого выше; типа-проверка — этот файл
  // грузится раньше ui.js, но к моменту, когда --sat реально меняется (безопасная зона Telegram
  // почти всегда приходит позже самой первой отрисовки), все deferred-скрипты уже выполнились.
  if (typeof retitleScreen==='function' && typeof screenName!=='undefined') requestAnimationFrame(()=>retitleScreen(screenName));
}
...
function resize(){
  ...
  /* 18.09.2026 (соло-аудит согласованности устройств, уточнение к CANVAS2D-DESKTOP-NOUTBUK §2.2):
     dprCap раньше пересчитывался только при смене настройки графики или авто-подстройке качества —
     никогда при resize(). Перетаскивание окна на монитор с другим DPI в режиме gfx:'auto' (или
     'ultra', та же ветка Math.min(raw,2.5)) оставляло старый потолок — не поломка, только размытие
     мимо новых пикселей монитора. gfxCap() читает Store.get('gfxTier',...) с кэшем — дёшево звать
     на каждый resize, не только на смену качества. */
  gfxCap();
