...
/* 23.09.2026: данные теста на A03 Core наконец есть — живой `cinemaTestStart/Stop`
   (tools/live-device.mjs) прямо на подключённом Samsung SM-A032F (tier:0): во время реальной
   записи avg:33.5 min:32.9 fps (7 замеров), кодек нашёлся и файл реально сохранился (VP9,
   software). 33.5 < CINEMA_HIGHLIGHT_MIN_FPS=40 — порог верно держит tier:0 вне авто-записи,
   калибровка подтверждена, не пересчитана в код (число уже безопасное, просто раньше не
   доказанное — не додумываю за владельца, стоит ли теперь подвинуть порог). */
