...
function nebulaField(h1,h2){
  ...
  /* звёздная пыль: сотни мелких точек, гуще у полосы; маскирует бандинг градиента.
     ...
     27.08.2026, позже тем же днём «Форма тоже разная»: раньше расстановка была ВСЕГДА
     одной и той же геометрией (полоса) — только координаты новые на каждый забег, форма
     композиции на глаз похожая. Владелец: хочет заметно другую форму от забега к забегу,
     без цены для слабых устройств. Тот же бюджет (240 точек), тот же разовый проход при
     пересборке текстуры (не в кадре, не в игровом цикле) — цена не меняется ни на йоту,
     меняется только формула. Сам сид R() выбирает форму — свой узор на забег, как и
     раньше, теперь ещё и своя форма. */
  const dustMode=Math.floor(R()*4);
  const dcx=fldW*(.3+R()*.4), dcy=fldH*(.3+R()*.4), dR=m*(.22+R()*.12); // общий центр/радиус для «кольца» и «грозди»
  const dust=[];
  for(let i=0;i<240;i++){
    let sx,sy;
    if(dustMode===0){ // полоса — прежняя расстановка
      const t=R(), near=R()<.6;
      sx=near? lerp(x0,x1,t)+(R()-.5)*fldW*.32 : R()*fldW;
      sy=near? lerp(y0,y1,t)+(R()-.5)*fldH*.24 : R()*fldH;
    } else if(dustMode===1){ // кольцо — пузырь туманности вокруг центра
      if(R()<.65){ const ang=R()*6.283, rad=dR*(.8+R()*.4);
        sx=dcx+Math.cos(ang)*rad; sy=dcy+Math.sin(ang)*rad*.7; }
      else { sx=R()*fldW; sy=R()*fldH; }
    } else if(dustMode===2){ // гроздь — плотное ядро, редеет к краям (R()*R() — гуще к центру)
      if(R()<.7){ const ang=R()*6.283, rad=dR*R()*R();
        sx=dcx+Math.cos(ang)*rad; sy=dcy+Math.sin(ang)*rad; }
      else { sx=R()*fldW; sy=R()*fldH; }
    } else { // россыпь — ровный лёгкий налёт, без явного центра
      sx=R()*fldW; sy=R()*fldH;
    }
