...
function fillGlyphPath(x,kind){
  ...
  switch(kind){
    ...
    case 'dash': { const k=1.15; // 27.08.2026 «Симметрия силуэтов»: было — три тонких шеврона,
      // заменены на сплошной клин-«остриё» (17.8% площади жетона, вровень со щитом — форма
      // проверена и не менялась с тех пор). 09.09.2026: владелец — «форму не трогай, просто
      // указать вперёд» — тот же силуэт повёрнут на 90°, остриё было направлено вправо
      // (6.6k,0), теперь вверх (0,-6.6k). Явное «да» получено словами (не молчание/«продолжай»).
      x.moveTo(-5*k,6*k); x.lineTo(-5*k,-2*k); x.lineTo(0,-6.6*k);
      x.lineTo(5*k,-2*k); x.lineTo(5*k,6*k); x.lineTo(0,2*k); x.closePath(); } break;
