...
const PREM_FX_MAP={
  ...
  // 09.09.2026 «физика/культура партия 2» (первая волна — физика/космос, 9 новых):
  cosComet:fxCosComet, cosNebula:fxCosNebula, cosGalaxyFloc:fxCosGalaxyFloc,
  cosLichtPos:fxCosLichtPos, cosLichtNeg:fxCosLichtNeg, cosPlateau:fxCosPlateau,
  cosPhyllo:fxCosPhyllo, cosSono:fxCosSono, cosFaraday:fxCosFaraday,
  // 09.09.2026 «физика/культура партия 2» (вторая волна — ремёсла/материалы, 26 новых;
  // Вуц/6-в-1/Тебриз этой же волны используют matWootz/chainmail6/persianTabriz выше):
  culTapa:fxCulTapa, culAndesFront:fxCulAndesFront, culTatreez:fxCulTatreez,
  culNavajoTwoGrey:fxCulNavajoTwoGrey, culNavajoGanado:fxCulNavajoGanado, culNavajoCrystal:fxCulNavajoCrystal,
  culCintemani:fxCulCintemani, culOlzii:fxCulOlzii, culAlkhanKhee:fxCulAlkhanKhee,
  culEthiopia:fxCulEthiopia, culJamdani:fxCulJamdani, culTnalak:fxCulTnalak, culRyijy:fxCulRyijy,
  matPatternWeld:fxMatPatternWeld, matChainmail4:fxMatChainmail4, matMarbleVein:fxMatMarbleVein,
  matFrost:fxMatFrost, matShellHex:fxMatShellHex, matNacre:fxMatNacre,
...
/* ===== 09.09.2026 «физика/культура партия 2» — перенос из macets/fizika-kultura-map-08-09-2026.html
   дословно (та же система координат BODY_PTS/-22..14/±16, что и clipShipBody — сверено координата
   в координату). Масштабы — из уже проверенного числом макета (см. project_fizika_kultura_maket_odobren_08_09
   в памяти: систематический проход, содержание доходит до 9-14 единиц тела из 16 доступных),
   ПОДЛЕЖИТ финальной проверке живым скриншотом до показа владельцу, как и остальные. ===== */
function fxCosComet(ctx,sk,nowMs){ // Хвосты комет — ионный (прямая линия) + пылевой (кривая), оба сразу
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%3400)/3400;
  const grow=0.35+0.65*Math.sin(p*Math.PI); // у перигелия хвостов почти нет, дальше от Солнца — реально удлиняются
  const g=ctx.createLinearGradient(0,0,0,13*grow);
  g.addColorStop(0,'hsla(200,90%,75%,.85)'); g.addColorStop(1,'hsla(200,90%,60%,0)');
  ctx.strokeStyle=g; ctx.lineWidth=0.5; ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(0,13*grow); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(0,0);
  for(let t=0.05;t<=10.8*grow;t+=0.3){ const rad=0.5*0.55*(t/6)*(t/6); ctx.lineTo(rad, -Math.abs(t)); }
  const g2=ctx.createLinearGradient(0,0,8.4*grow,-9.6*grow);
  g2.addColorStop(0,'hsla(35,70%,80%,.85)'); g2.addColorStop(1,'hsla(35,70%,70%,0)');
...
function fxCosGalaxyFloc(ctx,sk,nowMs){ // Flocculent — короткие рукава, каждый со своей скоростью (диф. ротация)
  ...
  ctx.fillStyle='hsla(45,90%,80%,.9)'; ctx.beginPath(); ctx.arc(0,0,1.3,0,6.283); ctx.fill();
  ctx.restore();
}
function fxCosLichtPos(ctx,sk,nowMs){ // Лихтенберг, позитивный разряд — дерево реально растёт волной
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%2200)/2200;
  const total=7, prog=p*total, revealDepth=total-Math.floor(prog), revealFrac=prog-Math.floor(prog);
  const rnd=mulberry32(11);
  (function branch(x,y,ang,len,depth){
    if(depth<=0||len<0.16) return;
    ...
    ctx.strokeStyle='hsla(205,80%,'+(70-depth*4)+'%,'+(0.9-depth*0.05).toFixed(2)+')'; ctx.lineWidth=len*0.5;
    ctx.beginPath(); ctx.moveTo(x,y); ctx.lineTo(x2,y2); ctx.stroke();
    if(partial<1) return;
    const nb=1+Math.floor(rnd()*2);
    for(let k=0;k<nb;k++){ const da=(rnd()-0.5)*Math.PI/2.2; branch(x2,y2,ang+da,len*0.72,depth-1); }
  })(0,10.4,-Math.PI/2,4.4,total);
  ctx.restore();
}
function fxCosLichtNeg(ctx,sk,nowMs){ // Лихтенберг, негативный разряд — кольца расходятся, как круги на воде
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%2600)/2600;
  for(let i=1;i<=6;i++){
    const rr=((i-1+p)%6)+1;
    ctx.strokeStyle='hsla(200,70%,70%,'+Math.max(0,0.9-rr*0.12).toFixed(2)+')'; ctx.lineWidth=0.16;
    ctx.beginPath(); ctx.arc(0,0,rr*1.6,0,6.283); ctx.stroke();
  }
  ctx.restore();
}
...
function fxCosSono(ctx,sk,nowMs){ // Сонолюминесценция — пузырёк растёт ~10x, резко схлопывается, вспышка на пике
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const cyclePos=(nowMs%2600)/2600;
  let r;
  if(cyclePos<0.75){ r=0.8+(cyclePos/0.75)*8; } else { const c=(cyclePos-0.75)/0.25; r=8.8-(c*8.6); }
  ctx.strokeStyle='hsla(200,70%,75%,.8)'; ctx.lineWidth=0.24;
  ctx.beginPath(); ctx.arc(0,0,r,0,6.283); ctx.stroke();
  if(cyclePos>0.93){
    const flashA=1-(cyclePos-0.93)/0.07;
    const g=ctx.createRadialGradient(0,0,0,0,0,4);
    g.addColorStop(0,'hsla(200,100%,90%,'+flashA.toFixed(2)+')'); g.addColorStop(1,'hsla(200,100%,80%,0)');
    ctx.fillStyle=g; ctx.beginPath(); ctx.arc(0,0,4,0,6.283); ctx.fill();
  }
  ctx.restore();
}
function fxCosFaraday(ctx,sk,nowMs){ // Волны Фарадея — квадратная решётка, честная субгармоника (половина частоты)
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%2400)/2400;
  const sub=Math.sin(p*Math.PI);
  for(let i=-3;i<=3;i++){
    for(let j=-3;j<=3;j++){
      const up=(i+j)%2===0;
      const r=1.3+(up?1:-1)*0.4*sub;
      ctx.fillStyle=up?'hsla(195,80%,70%,.8)':'hsla(195,40%,40%,.6)';
      ctx.beginPath(); ctx.arc(i*3.4,j*3.4,Math.max(0.4,r),0,6.283); ctx.fill();
    }
  }
  ctx.restore();
}
/* 09.09.2026, владелец («добавил анимацию квазикристаллу? да»): было медленное общее
   вращение — заменено на честный реальный процесс: настоящая электронная дифракция
   копится пятно за пятном по мере накопления электронов на детекторе (классический опыт
   единичных электронов), не появляется готовой сразу. У каждого из 40 пятен — свой
...
function fxBioBeetlePol(ctx,sk,nowMs){ // настоящий механизм (не украшение): через один фильтр круговой поляризации ярко, через другой — ПОЛНОСТЬЮ чёрное
  ...
  if(bright>0.04){
    /* 14.09.2026, владелец, живой скрин явления «Жук-скарабей»: «что это за белый свет
       квадратный?» — причина найдена и подтверждена живьём (Browser pane, круглый clip
       vs квадрат — с обрезкой диагональ пропадает полностью): блик ниже поворачивался
       (ctx.rotate) целым КВАДРАТОМ (fillRect 18×18) с линейным градиентом — его угол
       (9√2≈12.7 от центра) в определённые моменты цикла вылезал за пределы круглых колец
       (r≤9), это и читалось как диагональная полоса. Круглая обрезка перед заливкой —
       тот же смысл, что уже кольца сами по себе: блик остаётся внутри своей окружности,
       никогда не торчит углом. Страж 210. */
    ctx.save(); ctx.rotate(p*Math.PI*2);
    ctx.beginPath(); ctx.arc(0,0,9,0,6.283); ctx.clip();
...
// 09.09.2026 «физика/культура партия 2» (вторая волна — ремёсла/материалы, 26 новых
// функций; ещё 3 варианта той же партии — Вуц/6-в-1/Тебриз — переиспользуют функции
// выше без изменений). Перенос дословный из fizika-kultura-map-08-09-2026.html: тот же
...
function fxCulTatreez(ctx,sk,nowMs){ // Палестинская татрииз — счётный крест растёт кольцами от центра
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%3400)/3400;
  ctx.save(); ctx.scale(20,20);
  const cell=0.16;
  const ring=1+Math.floor(p*4);
  for(let r=1;r<=Math.min(ring,4);r++){
    const stitchesShown = r<ring ? 8 : Math.max(1,Math.floor((p*4-(ring-1))*8));
    for(let k=0;k<stitchesShown;k++){
      const ang=k*Math.PI/4;
      const x=Math.cos(ang)*r*cell, y=Math.sin(ang)*r*cell;
      ctx.fillStyle=r%2===0?'#c94f4f':'#e8ecf7';
...
function fxCulNavajoGanado(ctx,sk,nowMs){ // Навахо, Ganado — ступенчатые ромбы нарастают от края
  ...
  const p=(nowMs%3200)/3200;
  ctx.save(); ctx.scale(10,10);
  const ringShow=1+p*3;
  for(let ring=3; ring>=1; ring--){
    if(ring>ringShow+0.001) continue;
    const partial = ring>Math.floor(ringShow)? (ringShow-Math.floor(ringShow)) : 1;
    ctx.fillStyle = ring%2===0?'#1a1a1a':'#e8ecf7';
    const s=ring*0.32*Math.max(0.15,partial);
...
function culCintemaniUnit(ctx,cx,cy,scale){
  ...
  [[0,-1.7],[-1.4,0.6],[1.4,0.6]].forEach(([x,y])=>{ ctx.beginPath(); ctx.arc(x,y,0.85,0,6.283); ctx.fill(); });
  ctx.strokeStyle='#2a6a4a'; ctx.lineWidth=0.28;
  for(let i=0;i<2;i++){
    ctx.beginPath();
    for(let t=-2.3;t<=2.3;t+=0.1){ const x=t, y=2.3+i*0.75+Math.sin(t*3)*0.35; t===-2.3?ctx.moveTo(x,y):ctx.lineTo(x,y); }
    ctx.stroke();
  }
  ctx.restore();
}
...
function fxCulOlzii(ctx,sk,nowMs){ // Монгольский войлок, Өлзий — замкнутый узел трассируется от начала до узла
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%2600)/2600;
  ctx.save(); ctx.scale(18,18);
  ctx.strokeStyle='#e8c860'; ctx.lineWidth=0.06; ctx.lineCap='round';
  ctx.beginPath();
  const frac=Math.max(0.02,(p%1));
  for(let t=0;t<=frac;t+=0.01){
    const th=t*Math.PI*4;
    const r=0.42+0.14*Math.sin(th*2);
    const x=r*Math.cos(th), y=r*Math.sin(th)*0.7;
    t===0?ctx.moveTo(x,y):ctx.lineTo(x,y);
  }
  ctx.stroke();
  ctx.restore(); ctx.restore();
}
function fxCulAlkhanKhee(ctx,sk,nowMs){ // Монгольский войлок, Алхан хээ — открытый меандр, повороты только 90°
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%2600)/2600;
  ctx.save(); ctx.scale(10,10);
  const moves=['R','U','R','D','R','U','U','R','D','D','R','U','R'];
  let x=-1.1,y=-0.3;
  const shown=Math.max(1,Math.floor(p*moves.length));
  ctx.strokeStyle='#e8c860'; ctx.lineWidth=0.09; ctx.lineJoin='round';
  ctx.beginPath(); ctx.moveTo(x,y);
  moves.slice(0,shown).forEach(m=>{ if(m==='R') x+=0.18; else if(m==='U') y-=0.18; else if(m==='D') y+=0.18; ctx.lineTo(x,y); });
  ctx.stroke();
  ctx.restore(); ctx.restore();
}
function fxCulEthiopia(ctx,sk,nowMs){ // Эфиопский плетёный крест «вечность» — плетение прокладывается луч за лучом
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%3000)/3000;
  ctx.save(); ctx.scale(22,22);
  ctx.strokeStyle='#d8b860'; ctx.lineWidth=0.045; ctx.lineCap='round';
  const armFrac=Math.min(1,p*2);
  [[-0.55,0,0.55,0],[0,-0.55,0,0.55]].forEach(([x0,y0,x1,y1])=>{
    ctx.beginPath(); ctx.moveTo(x0,y0); ctx.lineTo(x0+(x1-x0)*armFrac, y0+(y1-y0)*armFrac); ctx.stroke();
  });
  const braidFrac=Math.max(0,Math.min(1,(p-0.5)*2));
  const braidsShown=Math.floor(braidFrac*4);
  for(let i=0;i<braidsShown;i++){
    const ang=i*Math.PI/2;
    ctx.save(); ctx.rotate(ang);
    ctx.beginPath();
    for(let t=0;t<=1;t+=0.05){ const r=0.2+t*0.35; const x=r, y=Math.sin(t*Math.PI*3)*0.06; t===0?ctx.moveTo(x,y):ctx.lineTo(x,y); }
...
function tnalakBandSprite(colIdx){
  ...
  /* 14.09.2026 (владелец, живой скрин: превью «размыто полностью, не только край») —
     было blur(1.5px) на полосе высотой 2px: радиус смазывания больше самой полосы,
     центр никогда не доходил до сплошного цвета (страж 209 измерил: 123/255 альфа
     в центре). Три варианта (1.5/0.7/0.4px) проверены живьём в браузере на реальном
     кадре — 0.4px даёт чёткую полосу с мягким, но не съедающим всё краем, ближе всего
     к описанию Ikat в факте скина. Число измерено, не на глаз — 0.7px неожиданно
     смазывал сильнее из-за наложения соседних полос, 1.5px (было) хуже всех. */
  g.filter='blur(0.4px)';
...
function fxMatPatternWeld(ctx,sk,nowMs){ // Дамаскирование — слои РАЗНЫХ сталей проступают по мере проковки-скручивания
  ...
  const p=(nowMs%3000)/3000;
  ctx.save(); ctx.scale(10,10);
  const shown=Math.floor(p*17)+1;
  for(let i=-8;i<=8;i++){
    if(i+9>shown) continue;
    ctx.strokeStyle=i%2===0?'hsla(210,10%,75%,.85)':'hsla(210,10%,35%,.85)'; ctx.lineWidth=0.05;
    ctx.beginPath(); ctx.moveTo(-1.3,i*0.09); ctx.lineTo(1.3,i*0.09+0.3); ctx.stroke();
  }
  ctx.restore(); ctx.restore();
}
...
function fxMatMarbleVein(ctx,sk,nowMs){ // Мраморные жилы — трещина реально растёт и ветвится со временем (геологический процесс)
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%2600)/2600;
  ctx.save(); ctx.scale(5.5,5.5);
  const rnd=mulberry32(21);
  const total=6, prog=p*total, revealDepth=total-Math.floor(prog), revealFrac=prog-Math.floor(prog);
  function crack(x,y,ang,len,depth){
    if(depth<=0||len<0.03) return;
    if(depth<revealDepth) return;
    const partial= depth===revealDepth? revealFrac : 1;
    const x2=x+Math.cos(ang)*len*partial, y2=y+Math.sin(ang)*len*partial;
    ctx.strokeStyle='rgba(90,90,95,'+(0.5+depth*0.05).toFixed(2)+')'; ctx.lineWidth=len*0.25;
    ctx.beginPath(); ctx.moveTo(x,y); ctx.lineTo(x2,y2); ctx.stroke();
    if(partial<1) return;
    if(rnd()<0.6) crack(x2,y2,ang+(rnd()-0.5)*1.4,len*0.6,depth-1);
    crack(x2,y2,ang+(rnd()-0.5)*0.6,len*0.75,depth-1);
  }
  crack(-1.1,-0.9,0.4,0.5,total);
...
function fxCulBasketCoil(ctx,sk,nowMs){ // Плетение корзин, койлинг — спираль сшивается виток за витком от центра
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%3200)/3200;
  ctx.save(); ctx.scale(11,11);
  ctx.strokeStyle='#c9a860'; ctx.lineWidth=0.08;
  ctx.beginPath();
  const frac=Math.max(0.02,p);
  for(let t=0;t<=frac;t+=0.01){ const th=t*Math.PI*8, r=t*1.1; const x=r*Math.cos(th), y=r*Math.sin(th); t===0?ctx.moveTo(x,y):ctx.lineTo(x,y); }
  ctx.stroke();
  ctx.restore(); ctx.restore();
}
function fxCulBasketPlait(ctx,sk,nowMs){ // Плетение корзин, плейтинг — ленты прокладываются через одну, полоса за полосой
  ctx.save(); clipShipBody(ctx); ctx.translate(0,-4);
  const p=(nowMs%3200)/3200;
  ctx.save(); ctx.scale(10,10);
  const lines=[]; for(let i=-6;i<=6;i++) lines.push(i);
  const shown=Math.floor(p*lines.length*2)+1;
  let count=0;
  for(const i of lines){
    if(count<shown){ ctx.strokeStyle=i%2===0?'#e8c878':'#8a6838'; ctx.lineWidth=0.09;
      ctx.beginPath(); ctx.moveTo(-1.3,i*0.16); ctx.lineTo(1.3,i*0.16); ctx.stroke(); }
    count++;
    if(count<shown){ ctx.strokeStyle=i%2===0?'#e8c878':'#8a6838'; ctx.lineWidth=0.09;
      ctx.beginPath(); ctx.moveTo(i*0.16,-1.3); ctx.lineTo(i*0.16,1.3); ctx.stroke(); }
    count++;
  }
  ctx.restore(); ctx.restore();
}
...
function fxCulPersianIsfahan(ctx,sk,nowMs){ // Персидский ковёр, Исфахан — тончайший медальон с пальметтами и вазами
  ctx.save(); clipShipBody(ctx);
  const p=(nowMs%3800)/3800;
  culPersianCity(ctx, 350, true, p);
  ctx.restore();
}
function fxCulPersianKashan(ctx,sk,nowMs){ // Персидский ковёр, Кашан — медальон и арабески
  ctx.save(); clipShipBody(ctx);
  const p=(nowMs%3800)/3800;
  culPersianCity(ctx, 20, true, p);
  ctx.restore();
}
function fxCulPersianQom(ctx,sk,nowMs){ // Персидский ковёр, Кум — тонкий шёлк, мелкие цветы
  ctx.save(); clipShipBody(ctx);
  const p=(nowMs%3800)/3800;
  culPersianCity(ctx, 330, true, p);
  ctx.restore();
}
function fxCulPersianNain(ctx,sk,nowMs){ // Персидский ковёр, Наин — кремовый фон, синий медальон
  ctx.save(); clipShipBody(ctx);
  const p=(nowMs%3800)/3800;
  culPersianCity(ctx, 215, false, p);
  ctx.restore();
}
...
function nfPanOffset(tN,distN,marginW,marginH){ // 22.08.2026 «Небо тоже плывёт»: главная текстура стояла на месте почти весь забег —
  ...
  // 20.09.2026 «Многоярусный станок» (владелец, вдохновлено техникой Норштейна — дальний
  // слой едет медленнее ближнего): раньше небо плыло ТОЛЬКО от S.time, с одной скоростью
  // что на старте (speed 3.4), что на потолке (speed 8.0) — звёзды фона так уже умеют
  // давно (render.js: s.y зависит от S.speed*(1+s.z)), а небо было к скорости полёта вообще
  // глухо. distN — это S.dist (пройденная дистанция), она уже сама по себе растёт
  // пропорционально скорости (game.js: S.dist+=S.speed*dt*S.timeScale*8) и уже правильно
  // замирает на паузе/вне полёта (тот же источник, что и S.time) — поэтому просто добавляем
  // её как вторую, отдельную от времени, фазу: небо теперь ощутимо ленивее на старте и
  // заметно живее на скорости, но НЕ быстрее уже одобренного максимума (коэффициенты
  // подобраны так, что чистый time-компонент остаётся прежним, dist-компонент добавляется
  // поверх). Числа проверены коротким скриптом вне игры ДО правки: на одинаковых 40с полёта
  // разница фазы между speed=3.4 и speed=8.0 ≈19% оборота (заметно на глаз, не резко), за
  // долгий 180с забег на средней скорости — ~3.4 оборота, без скачков (sin/cos всегда в
  // [-1,1], разгон дистанции не может увести офсет за границы запаса холста).
  const distX = distN*0.0008, distY = distN*0.0004;
  return { x: marginW*(0.5+0.5*Math.sin(tN*.08+distX)), y: marginH*(0.5+0.5*Math.cos(tN*.06+distY)) };
}
...
function drawNebulas(h1,h2,tN,lowPower){
  ...
  const pan=nfPanOffset(tN, S.dist||0, nf.marginPxX||0, nf.marginPxY||0);
...
function fillGlyphPath(x,kind){
  ...
  switch(kind){
    ...
    case 'dash': // 09.09.2026 «Раскол»: владелец выбрал по имени конкретный показанный
      // макет (три варианта присланы, «Раскол» назван прямо) — референт: таран разносит
      // препятствие (js/game.js:3037-3084, «ворота разбиваются об самолётик»). Шестиугольник
      // расколот на две половины со сдвигом в стороны — момент раскола, не стрела/остриё.
      // Проверено численно (scratchpad/taran-v3-check.js): 16.4% площади жетона (норма
      // 16-19%), охват 8.72. Взят ИМЕННО первый показанный вариант, без последующей
      // самовольной правки (жёсткая ломаная, поворот кусков) — владелец её отклонил.
      x.moveTo(-0.6,-8.7); x.lineTo(5.4,-4.7); x.lineTo(5.4,-1.7); x.lineTo(1.4,0.3);
      x.lineTo(-2.6,-2.7); x.lineTo(-6.6,-0.7); x.lineTo(-6.6,-4.7); x.closePath();
      x.moveTo(-5.4,0.7); x.lineTo(-1.4,-1.3); x.lineTo(2.6,1.7); x.lineTo(6.6,-0.3);
      x.lineTo(6.6,3.7); x.lineTo(0.6,7.7); x.lineTo(-5.4,3.7); x.closePath();
      break;
...
function drawGlyph(ctx,kind){
  ...
  } else if(kind==='slowmo'){ // 09.09.2026: циферблат → рябь времени (владелец процитировал
    // Стюарта Чейза, «тирания слов» — глиф должен рисовать РЕФЕРЕНТ (что бонус реально
    // делает: js/game.js:2237,2953,3146 — S.timeScale падает до 0.45 на 6с, время вокруг
    // растягивается), а не символ самого СЛОВА «медленно» (часы/черепаха/улитка — все
    // отклонены владельцем как нечитаемые). Три концентрических кольца + точка-исток,
    // как круги на воде, застывшие кадром. Проверено численно (scratchpad/
    // referent-icons-check.js): 17.3% площади жетона (норма 16-19%), охват 8.4.
    ctx.fillStyle=dcol;
    ctx.beginPath(); ctx.arc(0,0,8.4,0,6.283); ctx.arc(0,0,6.6,0,6.283); ctx.fill('evenodd');
    ctx.beginPath(); ctx.arc(0,0,5.4,0,6.283); ctx.arc(0,0,3.9,0,6.283); ctx.fill('evenodd');
...
function draw(){
  ...
  /* 28.08.2026 «Живой замер плотности», итог: было 48/90/140 (эталон при ширине 390) — влад-
     ...
     11.09.2026 «Самые дорогие объекты» (владелец, полный аудит — единственная настоящая
     находка на 15 проверенных): было 2 планки на 4 фактических уровня — isLowPowerDevice()
     (класс железа при загрузке, не Q.level) давал ~96, «Средний» и «Высокий» делили одно
     число (180). Теперь 4 честные ступени по Q.level — том же сигнале, что уже двигает
     скины/вспышки/следы после сегодняшних правок. 180 (Высокий) и весь пул (Ультра) —
     НЕ новые числа, ровно то, что владелец одобрил 28.08.2026 выше. «Дно» (90) и «Средний»
     (140) — новые, посчитаны от измеренной живьём цены звезды (2.74мкс/шт, замер 11.09.2026,
     311 звёзд ≈ 0.85мс) и подтверждены владельцем явно, не выдуманы на глаз. */
  const STAR_TIER=[90,140,180]; // дно/средний/высокий — при wScale=1 (ширина 390); ультра — весь пул отдельно ниже
  let nStars = (Q.level>=3) ? bgStars.length : Math.min(Math.round(STAR_TIER[Q.level]*wScale), bgStars.length); // На слабых телефонах убираем лишние тысячи вычислений на бэкграунде
  ...
  finishDraw(); // 13.09.2026 «Ворота финиша»: тот же приём, что у goldDraw() выше — арка на подходе, цветной салют на пересечении
  ...
  for (const o of obstacles){
    ...
    if (o.kind==='debris'){ // семья обломков (v1.105.0 «Свет и дым»): один смысл «рукотворный
      ...
      for (const sgn of SGN2){
        ...
        if(sh){ // внутреннее кольцо пилона — «радужка» (17.09.2026, владелец: «шарик как зрачок,
          // между ним и ободом ещё кольцо, оно может расширяться и сужаться, как глаза» — люди
          // быстро замечают глаза, тот же язык уже есть у Мины/Ловца рядом в этом файле).
          // Тот же o.ph, что уже двигает «дыхание» просвета (строка выше в game.js) — радужка
          // дышит в такт воротам, не отдельным таймером. Доля от pr (0.35..0.65), не абсолютные
          // px — безопасно при любом pr, никогда не долетает ни до зрачка (r=3), ни до обода (pr).
          ctx.strokeStyle='rgba(122,140,255,.35)'; ctx.lineWidth=1;
          ctx.beginPath(); ctx.arc(sgn*g2,0,pr*(.5+.15*Math.sin(o.ph)),0,6.283); ctx.stroke();
        }
        // «Зрачок» мигает — тот же приём (globalAlpha=.55+.45*sin), что уже у Мины/Ловца в этом
        // файле, тем же ph, что и радужка выше, чтобы оба «моргали» в одном ритме, не вразнобой.
        ctx.globalAlpha=.55+.45*Math.sin(o.ph*2);
...
function renderTrailPattern(c, style, col){
  ...
  } else if(style==='sparks'){ // Искры: точки следа
    /* 13.09.2026 (владелец, живой скрин Коллекции): «одна яркая вспышка» здесь раньше рисовалась
       СПЛОШНЫМ непрозрачным белым кругом (r≈4.5) у самого носа — в живом полёте тот же эффект
       (render.js, ветка p.trailFx==='sparks' выше) честно редкий и на миг, gated по p.flashAt,
       но в статичной заморозке для плитки/окна превью не бывает «редко» — круг был виден ВСЕГДА,
       тем же почти-белым цветом и почти тем же размером/местом, что и сам нос-ориентир
       (#eaf2ff, ui.js) поверх него — полностью его закрывал. У всех остальных следов на том же
       экране нос виден, у «Искры» — нет. Владелец: «удали её значит она сломана» — убрана из
       заморозки совсем; сам редкий эффект в живом полёте не тронут, страж 186. */
...
/* 15.09.2026 «Золотая спираль, по-настоящему непрерывная»: 7 квадратов Фибоначчи
   (1,1,2,3,5,8,13) выложены «черепашьей» конструкцией — каждый следующий квадрат
   пристыкован гранью к растущему прямоугольнику предыдущих, дуга каждого квадрата
   стартует ровно там, где закончилась дуга предыдущего. Проверено численно (скрипт,
   scratchpad, не сохранён): 0 разрывов из 6 стыков, макс. отклонение <1e-9. Раньше
   центры/позиции дуг были подобраны на глаз (FIB-массив) и НЕ стыковались вовсе —
   отсюда «крошечный крючок» вместо спирали, жалоба владельца 15.09.2026. Страж
   guardGoldenSpiralArcsConnect (tests/guard.mjs) держит это свойство навсегда. */
const GOLDEN_SPIRAL_ARCS=[
  {r:1,  cx:0,  cy:1,  a0:-Math.PI/2},
  {r:1,  cx:0,  cy:1,  a0:0},
  {r:2,  cx:0,  cy:0,  a0:Math.PI/2},
  {r:3,  cx:1,  cy:0,  a0:Math.PI},
  {r:5,  cx:1,  cy:2,  a0:-Math.PI/2},
  {r:8,  cx:-2, cy:2,  a0:0},
  {r:13, cx:-2, cy:-3, a0:Math.PI/2},
];
...
function renderFlashPattern(c, style, p, col){
  ...
  switch(style){
    ...
    case 'goldenSpiral': {
      /* 15.09.2026 (владелец, живой скрин из Тюнинга: «какой-то блок внутри», нашёл сам —
         прежние координаты FIB были подобраны на глаз и НЕ образовывали одну спираль: конец
         каждой дуги не совпадал с началом следующей (проверено численно — 6 разрывов из 6
         пар), рисовался крошечный разорванный крючок вместо нау́тилуса. Переписано «черепашьей»
         конструкцией — дуга каждого следующего квадрата стартует ровно там, где кончилась
         предыдущая, гарантия непрерывности по построению, не подбором координат. GOLDEN_SPIRAL_ARCS
         вынесен в module-scope константу и проверяется стражом (tests/guard.mjs) на разрывы. */
      c.strokeStyle=col(1-p); c.lineWidth=1;
      GOLDEN_SPIRAL_ARCS.forEach(a=>{
        const SC=1.6*p, r=a.r*SC, x=a.cx*SC, y=a.cy*SC;
        c.beginPath(); c.arc(x,y,r,a.a0,a.a0+Math.PI/2); c.stroke();
      });
      break;
    }
    case 'apollonian': {
...
function drawPlane(sh,nowMs){
  ...
  /* 09.09.2026, владелец (скриншот с яркой оранжевой обшивкой): «белая линия на правом крыле
     видна... смотрится как баг». Тот же класс жалобы, что уже был у блик-эллипса носа
     (31.08.2026, «я думал что это какая-то ошибка... он мешает») — та деталь была убрана
     целиком, эта — тоже. Была только на графике «Ультра» (Q.level>=3). */
...
function loop(t){
  ...
  if (typeof idleHintTick==='function') idleHintTick(dt); // 24.09.2026: подсказка при бездействии на главной — тикает вместе с общим циклом, тот же dt
