...
/* 16.09.2026 «Биполярная туманность — низ обрезан ровной линией» (владелец, три скрина подряд,
   в итоге — его же запись с телефона, покадрово, увеличенный кроп подтвердил: нижняя доля
   упирается в ПРЯМУЮ горизонтальную линию, не в свою естественную округлую границу — clip
   ANGAR_PV_FIT_COLOR (см. её комментарий выше) измерен под ДРУГУЮ задачу — горизонтальное/
   диагональное переполнение половины сцены, не вертикальный упор в язычок «Cosmogram». Смешивать
   две разных причины в одном числе — терять смысл обеих таблиц. Живой замер (getImageData,
   clip временно отключён, весь цикл «дыхания» 4200мс, шаг 30мс, OWNER_TG 361×667): на пике
   («дыхание» почти не сжато) нижняя доля перехлёстывала границу язычка на 49px. ×0.5 —
   измеренный запас ~11px на худшем кадре цикла, верх при этом остаётся с большим запасом
   (~70-100px, не упирается никуда). Только id=62 — единственный узор с формой «две доли
   разъезжаются по вертикали», у всех остальных 56 узоров в ANGAR_PV_FIT_COLOR такой формы нет,
   доразмер не нужен. */
const ANGAR_PV_FIT_VERT={62:0.5};
  ...
  if(W && H){
    ...
    if(x){
      ...
      /* 11.09.2026 (владелец, живой скрин с кругом: «явление заходит за линию, где золотой
         ...
         Резерв нужен только чтобы холст не резал ПОСЛЕДНИЙ пиксель впритык (нулевой зазор бы
         означал, что случайный кадр дыхания/поворота может лечь ровно на линию язычка и дать
         жёсткий прямой срез — тот самый баг 16.09.2026 у id=62). +3px — не «красивый зазор»,
         а честный технический буфер от жёсткого совпадения линий, дальше решает сам узор:
         большой (кольца, спираль) сам дотянется почти вплотную, маленький (пульсар и т.п.)
         останется маленьким, это его естественный размер, не баг. */
      const markElPv=$('angarPvZoomMark');
      const pvReserve=(markElPv ? markElPv.getBoundingClientRect().height : 25)+3;
      ...
      if(angarPvZoomCat==='color'){
        /* 11.09.2026, владелец: явление доходит до карточки факта, не зависает в пустоте
           ...
           каждое». Откачено обратно к 0.12 — общий множитель не трогаем.
        ...
        x.translate(0,Havail*0.12+(ANGAR_PV_ZOOM_EXTRA_DROP[item.id]||0));
