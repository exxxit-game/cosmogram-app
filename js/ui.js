  ...
  if(W && H){
    ...
    if(x){
      ...
      /* 11.09.2026 (владелец, живой скрин с кругом: «явление заходит за линию, где золотой
         ...
         правку размера язычка сам, без пересчёта здесь. +6px — тот же «не впритык к краю»
         запас, что уже даёт 92% (не 100%) половины сцены по бокам. */
      const markElPv=$('angarPvZoomMark');
      const pvReserve=(markElPv ? markElPv.getBoundingClientRect().height : 25)+6;
      x.beginPath(); x.rect(0,0,W,Math.max(0,H-pvReserve)); x.clip();
      x.translate(W/2,H/2);
      ...
      if(angarPvZoomCat==='color'){
        x.translate(0,H*0.12); // 11.09.2026, владелец: явление доходит до карточки факта, не зависает в пустоте посередине
