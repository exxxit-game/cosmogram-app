/* ============================================================
   ...
   — 27.08.2026: Q0 больше не молчит целиком — метеор и вспышка мили дёшевы, идут везде;
     станция (дороже) остаётся с Q2+, Q1 её тоже не получает;
...
const PLANET=(()=>{
  ...
  function sky(tN){
    ...
    /* 27.08.2026 «Что ещё бесплатно достанется дну»: раньше здесь стояло `if(Q.level===0)
       ...
       кэш-спрайт, без своих градиентов) — и попросил вернуть их дну. Станция НЕ возвращается
       вместе с ними — её собственный запрет чуть ниже (Q.level>=2) не трогаем, она заметно
       дороже (шесть кэш-градиентов при первом появлении, дольше держится в кадре). */
