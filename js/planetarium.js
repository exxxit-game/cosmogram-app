/* ============================================================
   ...
   — 27.08.2026: Q0 больше не молчит целиком — метеор и вспышка мили дёшевы, идут везде;
     19.09.2026: станция тоже больше не заперта за Q2+ — реальный замер после оптимизации
     скинов показал низкую цену, идёт на всех ярусах;
...
const PLANET=(()=>{
  ...
  function sky(tN){
    ...
    /* 27.08.2026 «Что ещё бесплатно достанется дну»: раньше здесь стояло `if(Q.level===0)
       return;` — весь модуль молчал на самом слабом уровне (закон был явно записан в шапке
       файла: «Q0 молчит»). Владелец спросил цену конкретно метеора и вспышки мили — оба
       редкие (метеор раз в 18-40с, вспышка раз в километр) и дешёвые, когда идут (один
       кэш-спрайт, без своих градиентов) — и попросил вернуть их дну. Станция тогда НЕ
       вернулась вместе с ними — собственный запрет чуть ниже был заметно дороже (шесть
       кэш-градиентов при первом появлении, дольше держится в кадре). 19.09.2026: запрет
       снят тоже — см. комментарий у самого запрета ниже, реальный замер после оптимизации
       скинов показал низкую цену. */
    ...
    // станция: редкая гостья глубины
    // 19.09.2026 (владелец, реальный замер на Samsung SM-A032F после оптимизации скинов:
    // 0.02мс/кадр в среднем, 2.6мс в момент появления, редко) — прежний потолок по ярусу
    // графики снят, тем же правилом, что уже применено к анимации «явления» в Коллекции:
    // дно теперь тянет так же, как топ. Ограничение было не про доступность игроку, а про
    // честность цены — цена оказалась низкой.
    {
      staT-=dt;
      if(staT<=0&&!sta){
  ...
  function rockTint(o){
    ...
    if(o.kind==='drift'){
      /* 09.09.2026 (владелец, макет drift_macet.png, «да»): старый серо-сиреневый почти
         сливался с половиной небес (риск-пары [216,256]/[291,331], светлота всего на
         2-16 пунктов выше пика неба) — заменён на Tol Purple (Paul Tol muted qualitative
         scheme, #AA4499), проверенный численно и через симуляцию дальтонизма против
         Мины/Ловца/Ворот/бонусов, коллизий нет. Три тона — та же роль, что у старых
         (лёгкая случайная вариация, не сигнал), не выдуманы на глаз: base ±10% к
         чёрному/белому от канонического hex. */
      o._tint=['#993d8a','#AA4499','#b357a3'][h%3<0?-h%3:h%3];
    }else{
