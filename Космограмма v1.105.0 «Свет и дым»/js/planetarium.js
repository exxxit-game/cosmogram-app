...
const PLANET=(()=>{
  ...
  return { sky, rockTint, planeFx, spark, engineK,
    // мостик стража: заглянуть внутрь и подтолкнуть редких гостей (тесты, не игра)
    _state:()=>({metT, met:!!met, staT, sta:!!sta, staX:sta?Math.round(sta.x):-1, staY:sta?Math.round(sta.y):-1, sparks:sparks.length, mileT, flash:!!flash}),
