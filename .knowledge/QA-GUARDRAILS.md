...
- ⚠️ **Неверно, исправлено 27.08.2026:** строка ниже утверждала «автоматический запуск через
  GitHub Actions CI» — `.github/workflows/` физически не существует в репозитории (подтверждено
  `ls`), весь прогон стенда сейчас РУЧНОЙ. Это не просто пробел документа, а описание
  несуществующего процесса как факта — см. `KNOWN-BUGS.md` («мало тегов релизов, нет CI»).
  Технически реализуемо без нарушения запрета на npm-зависимости в самой игре: глобальная
  установка Playwright в CI-раннере (`npm install -g playwright` + `npx playwright install
  --with-deps chromium`), не в `package.json` игры — см. `KNOWN-BUGS.md` за точным синтаксисом.
