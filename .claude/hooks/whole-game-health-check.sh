...
echo "  Supabase-токен доступным скрипту (management/service-role ключ), а это понижает"
echo "  безопасность против текущей схемы «только через MCP, только у агента» (весь вечер"
echo "  укреплялась именно она — заводить в файл ключ ради удобства одного отчёта было бы"
